...
\tcode{ptr} \changed{shall be a null pointer or
its value shall represent}{is a null pointer or
...
then \tcode{ptr} \changed{shall be}{is} a safely-derived pointer.

\pnum
\changed{\requires}{\expects}
If the \tcode{alignment} parameter is not present,
\tcode{ptr} \changed{shall have been}{was} returned by an allocation function
without an \tcode{alignment} parameter.
If present, the \tcode{alignment} argument
\changed{shall equal}{is equal to} the \tcode{alignment} argument
passed to the allocation function that returned \tcode{ptr}.
If present, the \tcode{size} argument
\changed{shall equal}{is equal to} the \tcode{size} argument
passed to the allocation function that returned \tcode{ptr}.
...
\begin{itemdescr}
\added{\pnum \mandates \tcode{!is_function_v<T> \&\& !is_void_v<T>} is \tcode{true}.}
...
\removed{The program is ill-formed if \tcode{T} is a function type
or \cv{}~\tcode{void}.}
...
\pnum
\changed{\requires}{\expects}
\tcode{p} \changed{shall not be}{is not} a null pointer.
...
\pnum 
\changed{\requires}{\expects}
\tcode{U} \changed{shall be}{meets the} \oldconcept{CopyConstructible} \added{requirements}.
