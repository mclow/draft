...
namespace std {
  ...
  public:
    ...
    // locale operations
    @\changed{basic_string<char>}{string}@ name() const;
...
\tcode{locale::classic()}.
\begin{note}
This constructor \added{yields a copy of the current global locale. It} is commonly used as the default value for arguments
...
\begin{itemdescr}
\ednote{The old P1 gets an \effects, and the old P2 gets merged into that. P3 split off from the old P2}

\pnum
\added{\effects} Sets the global locale to its argument.
...
\pnum
\expects
Either \tcode{tbl == nullptr} is true or \range{tbl}{tbl+table_size} is a valid range.
\end{addedblock}
...
\tcode{(from <= from_end \&\& to <= to_end)} \added{is}
well-defined and
\tcode{true};
\tcode{state} \added{is} initialized, if at the beginning of a sequence, or else equal to
...
\pnum
\changed{\requires}{\expects}
\tcode{(to <= to_end)} \added{is}
well-defined and \tcode{true}; state \added{is} initialized, if at the beginning of a sequence,
...
\pnum
\returns
A \changed{basic_string<char>}{\tcode{string}} \tcode{vec} used as a vector of integer values,
...
\begin{itemdescr}
\pnum
\changed{\requires}{\expects} \range{fmt}{fmtend} \changed{shall be}{is} a valid range.
...
\pnum
\changed{\requires}{\expects}
\tcode{t} \changed{shall point}{points} to an object.
...
namespace std {
  ...
  template<class charT>
    ...
    public:
      ...
      catalog open(const @\changed{basic_string<char>}{string}@& fn, const locale&) const;
    ...
    protected:
      ~messages();
      virtual catalog do_open(const @\changed{basic_string<char>}{string}@&, const locale&) const;
...
\begin{itemdecl}
catalog open(const @\changed{basic_string<char>}{string}@& name, const locale& loc) const;
\end{itemdecl}
...
\begin{itemdecl}
catalog do_open(const @\changed{basic_string<char>}{string}@& name, const locale& loc) const;
\end{itemdecl}
...
\pnum
\changed{\requires}{\expects}
\tcode{cat} \changed{shall be}{is} a catalog obtained from
\tcode{open()}
and not yet closed.
