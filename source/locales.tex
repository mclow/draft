...
\tcode{locale::global()}
\changed{shall affect}{affects} the value returned by
\tcode{locale()}.
...
\tcode{tbl} \added{is} either \changed{0}{nullptr} or \changed{an array of at least
\tcode{table_size}
elements.}{\tcode{[tbl, tbl+table\_size)} is a valid range.}
...
\ednote{Why can't we just remove this? The effects talk about 
"assigning those \tcode{struct tm} members" and "leaves the object pointed 
to by \tcode{t} unchanged" - those should be enough, I would think. 
This call was added for c++11, and it just said "t shall be dereferenceable", 
but that was changed via CWG232. This is the only \tcode{do\_get} call that takes
the object to be filled in as a pointer; all the others take references. }
