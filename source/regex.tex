...
\begin{itemdescr}
\removed{\pnum\effects  Constructs an object of class \tcode{regex_error}.}
...
\tcode{regex_traits<wchar_t>} \removed{shall be valid and shall} meet the
...
\begin{itemdescr}
\pnum
\changed{\requires}{\expects}  The value of \tcode{radix} \changed{shall be}{is} 8, 10, or 16.
...
\ednote{All through this sub-clause, please move the "Throws:" elements to be after the "Effects:" and "Ensures:" elements.}
\begin{itemdescr}
\pnum
\removed{\effects Constructs an object of class \tcode{basic_regex} that
does not match any character sequence.}
\added{\ensures \tcode{*this} does not match any character sequence.}
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\changed{\requires \tcode{p} shall not be a null pointer.}
        {\expects \range{p}{p+char_traits<charT>::length(p)} is a valid range.}

\pnum
\throws  \tcode{regex_error} if 
\changed{\tcode{p}}{\range{p}{p+char_traits<charT>::length(p)}} is not a valid regular expression.

\pnum
\effects  \removed{Constructs an object of class \tcode{basic_regex};} \changed{t}{T}he object's
internal finite state machine is constructed from the regular
expression contained in the \changed{array of \tcode{charT} of length
\tcode{char_traits<charT>::\brk{}length(p)} whose first element is
designated by \tcode{p}}{sequence of characters 
...
\begin{itemdescr}
\pnum
\removed{\requires \tcode{p} shall not be a null pointer.}
\added{\expects \range{p}{p+len} is a valid range.}

\pnum
\throws  \tcode{regex_error} if \changed{\tcode{p}}{\range{p}{p+len}} is not a valid regular expression.
...
\removed{\tcode{e} is in a valid state with unspecified value.}
\end{itemdescr}
...
\pnum
\effects \changed{Returns}{Equivalent to:} \tcode{\added{return} assign(ptr)}\changed{.}{;}
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\effects \changed{Returns}{Equivalent to:} \tcode{\added{return} assign(il.begin(), il.end())}\changed{.}{;}
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\effects \changed{Returns}{Equivalent to:} \tcode{\added{return} assign(p)}\changed{.}{;}
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\effects Equivalent to: \tcode{return *this = that\changed{.}{;}}
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\effects Equivalent to: \tcode{return *this = std::move(that)\changed{.}{;}}
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\changed{\returns}{\effects Equivalent to:} \tcode{\added{return} assign(string_type(ptr), f)}\changed{.}{;}
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\changed{\returns}{\effects Equivalent to:} \tcode{\added{return} assign(string_type(ptr, len), f)}\changed{.}{;}
\end{itemdescr}
...
\pnum
\effects \changed{Returns}{Equivalent to:} \tcode{\added{return} assign(string_type(first, last), f)}\changed{.}{;}
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\effects \changed{Same as}{Equivalent to:} \tcode{\added{return} assign(il.begin(), il.end(), f)}\changed{.}{;}
...
\pnum
\effects
Move constructs an object of class \tcode{match_results} from \tcode{m} satisfying the same postconditions as \tref{re.results.const}.
Additionally, the stored \tcode{Allocator} value is move constructed from \tcode{m.get_allocator()}.
\end{removedblock}

\begin{addedblock}
\pnum
\effects The stored \tcode{Allocator} value is move constructed from \tcode{m.get_allocator()}.

\pnum
\ensures {As specified in} \tref{re.results.const}.
\end{addedblock}
...
\begin{itemdescr}
\pnum
\changed{\requires}{\expects} \tcode{ready() == true}.
...
\begin{itemdescr}
\pnum
\changed{\requires}{\expects} \tcode{ready() == true} and \tcode{OutputIter} \changed{shall meet}{meets} the requirements for a
...
\removed{The type} \tcode{BidirectionalIterator} \changed{shall meet}{meets} the
...
\pnum
\changed{\requires}{\expects}
\removed{Type} \tcode{BidirectionalIterator} \changed{shall meet}{meets} the
\oldconcept{BidirectionalIterator} requirements\iref{bidirectional.iterators}.
...
\begin{itemdescr}
\pnum
\changed{\requires}{\expects} Each of the initialization values of \tcode{submatches} \changed{shall be}{is}
\tcode{>= -1}.
