...
\tcode{regex_traits<wchar_t>} \changed{shall be}{are} valid and \removed{shall }meet the
...
\begin{itemdescr}
\pnum
\changed{\requires \tcode{ptr} shall not be a null pointer.}
        {\expects \range{ptr}{ptr+char_traits<charT>::length(ptr)} is a valid range.}

\ednote{Do we even need this requirement? We get it (implicitly) from the call to \tcode{assign}, which gets it from the string's constructor.}
...
\begin{itemdescr}
\pnum
\changed{\requires}{\expects} \tcode{ready() == true} and \added{that} \tcode{OutputIter} \changed{shall meet}{meets} the requirements for a
