...
\pnum
\tcode{D} \changed{shall}{meet} satisfy the
\oldconcept{CopyConstructible} (\tref{copyconstructible})
and \oldconcept{CopyAssignable} (\tref{copyassignable}) requirements.
...
\pnum
\mandates
\tcode{iterator_traits<InputIterator>::value_type} is a type that is convertible to \tcode{double}.
\end{addedblock}
...
are both types that are convertible to \tcode{double}.
\end{addedblock}
...
The \grammarterm{id-expression}s
  \tcode{iterator_traits<InputIteratorB>::value_type} and
  \tcode{iterator_traits<Input\-IteratorW>::value_type}
  are both convertible to \tcode{double}.
\end{addedblock}
...
\pnum
\constraints
The indicated operator can be applied to an operarand of type \tcode{T} and returns 
