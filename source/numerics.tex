...
\pnum
A separate floating-point environment \changed{shall be}{is} maintained for each thread. Each function
...
\begin{itemize}
\item the expression \tcode{reinterpret_cast<\cv{} T(\&)[2]>(z)} \changed{shall be}{is} well-formed,
\item \tcode{reinterpret_cast<\cv{} T(\&)[2]>(z)[0]} \changed{shall designate}{designates} the real part of \tcode{z}, and
\item \tcode{reinterpret_cast<\cv{} T(\&)[2]>(z)[1]} \changed{shall designate}{designates} the imaginary part of \tcode{z}.
\end{itemize}
...
\begin{itemize}
\item \tcode{reinterpret_cast<\cv{} T*>(a)[2*i]} \changed{shall designate}{designates} the real part of \tcode{a[i]}, and
\item \tcode{reinterpret_cast<\cv{} T*>(a)[2*i + 1]} \changed{shall designate}{designates} the imaginary part of \tcode{a[i]}.
\end{itemize}
...
\pnum
\ensures
\tcode{real() == re \&\& imag() == im} \added{is true.}
\end{itemdescr}
...
\begin{itemdescr}
\ednote{LWG 2714 proposes rewriting this entire description, so I've made a minimal change here.}

\pnum
\changed{\requires}{\expects}
The input values \changed{shall be}{are} convertible to
\tcode{T}.
...
\pnum
\changed{\requires}{\expects}
\tcode{rho} \changed{shall be}{is} non-negative and non-NaN\@. \tcode{theta} \changed{shall be}{is} finite.
...
\end{itemize}
\end{removedblock}
This function \changed{shall be}{is} \tcode{constexpr} if and only if
...
\pnum
A class \tcode{S}
meets the requirements of a seed sequence
...
\indextext{\idxcode{operator>>}!random number distribution requirement}
  ...
  & Restores from \tcode{is}
    ...
    \changed{\requires}{\expects} \tcode{is} provides a textual representation
    that was previously written
...
\begin{itemdescr}
\pnum\changed{\effects}{\remarks} \removed{Constructs a \tcode{linear_congruential_engine} object.}
...
\begin{itemdescr}
\pnum\changed{\effects}{\remarks} \removed{Constructs a \tcode{mersenne_twister_engine} object.}
...
\begin{itemdescr}
\pnum\changed{\effects}{\remarks} \removed{Constructs a \tcode{subtract_with_carry_engine} object.}
 Sets the values of
 $X_{-r}, \dotsc, X_{-1}$,
...
\pnum\required
 ...
 \changed{shall produce}{produces} the value $1043618065$.
\end{itemdescr}
...
\pnum\required
 ...
 \changed{shall produce}{produces} the value $399268537$.
\end{itemdescr}
...
\pnum\required
 ...
 \changed{shall produce}{produces} the value $4123659995$.
\end{itemdescr}
...
\pnum\required
 ...
 \changed{shall produce}{produces} the value $9981545732273789042$.
\end{itemdescr}
...
\pnum\required
 ...
 \changed{shall produce}{produces} the value
 $7937952$.
\end{itemdescr}
...
\pnum\required
 ...
 \changed{shall produce}{produces} the value $1112339016$.
\end{itemdescr}%
...
\begin{itemdescr}
\pnum\changed{\effects}{\remarks} \removed{Constructs a \tcode{random_device}
...
\begin{addedblock}
\pnum\ensures \tcode{v.empty()} is true.
\end{addedblock}
...
\begin{itemdescr}
\pnum
\changed{\requires}{\mandates} \tcode{T} \changed{shall be}{is} an integer type.
...
\pnum
\mandates
  \tcode{iterator_traits<InputIterator>::value_type} is an integer type.
\end{addedblock}

\pnum
\changed{\requires}{\expects}
  \tcode{InputIterator} \changed{shall meet}{meets} the
...
\pnum\effects \changed{Constructs a \tcode{seed_seq} object}{Initializes \tcode{v}}
...
\pnum
\changed{\requires}{\expects} \tcode{RandomAccessIterator}
  \changed{shall meet}{meets} the
...
\pnum\changed{\requires}{\expects}
  \tcode{OutputIterator} \changed{shall meet}{meets} the
...
\pnum\changed{\effects}{\remarks} \removed{Constructs a \tcode{uniform_int_distribution} object;}
 \tcode{a} and \tcode{b}
...
\pnum\changed{\effects}{\remarks} \removed{Constructs a \tcode{uniform_real_distribution} object;}
 \tcode{a} and \tcode{b}
 correspond to the respective parameters of the distribution.
\end{itemdescr}
...
\pnum\changed{\effects}{\remarks} \removed{Constructs a \tcode{bernoulli_distribution} object;}
 \tcode{p}
...
\pnum\changed{\effects}{\remarks} \removed{Constructs a \tcode{binomial_distribution} object;}
 \tcode{t} and \tcode{p}
 correspond to the respective parameters of the distribution.
\end{itemdescr}
...
\pnum\changed{\effects}{\remarks} \removed{Constructs a \tcode{geometric_distribution} object;}
 \tcode{p}
 corresponds to the parameter of the distribution.
\end{itemdescr}
...
\pnum\changed{\effects}{\remarks} \removed{Constructs a \tcode{negative_binomial_distribution} object;}
 \tcode{k} and \tcode{p}
 correspond to the respective parameters of the distribution.
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\changed{\requires}{\expects} $0 < \tcode{mean}$.

\pnum
\changed{\effects}{\remarks} \removed{Constructs a \tcode{poisson_distribution} object;}
...
\begin{itemdescr}
\pnum
\changed{\requires}{\expects} $0 < \tcode{lambda}$.

\pnum
\changed{\effects}{\remarks} \removed{Constructs an \tcode{exponential_distribution} object;}
...
\begin{itemdescr}
\pnum
\changed{\requires}{\expects} $0 < \tcode{alpha}$ and $0 < \tcode{beta}$.

\pnum
\changed{\effects}{\remarks} \removed{Constructs a \tcode{gamma_distribution} object;}
...
\begin{itemdescr}
\pnum
\changed{\requires}{\expects} $0 < \tcode{a}$ and $0 < \tcode{b}$.

\pnum
\changed{\effects}{\remarks} \removed{Constructs a \tcode{weibull_distribution} object;}
\tcode{a} and \tcode{b}
correspond to the respective parameters of the distribution.
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\changed{\requires}{\expects} $0 < \tcode{b}$.

\pnum
\changed{\effects}{\remarks} \removed{Constructs an \tcode{extreme_value_distribution} object;}
\tcode{a} and \tcode{b}
correspond to the respective parameters of the distribution.
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\changed{\requires}{\expects} $0 < \tcode{stddev}$.

\pnum
\changed{\effects}{\remarks} \removed{Constructs a \tcode{normal_distribution} object;}
...
\begin{itemdescr}
\pnum
\changed{\requires}{\expects} $0 < \tcode{s}$.

\pnum
\changed{\effects}{\remarks} \removed{Constructs a \tcode{lognormal_distribution} object;}
\tcode{m} and \tcode{s}
correspond to the respective parameters of the distribution.
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\changed{\requires}{\expects} $0 < \tcode{n}$.

\pnum
\changed{\effects}{\remarks} \removed{Constructs a \tcode{chi_squared_distribution} object;}
...
\pnum
\changed{\effects}{\remarks} \removed{Constructs a \tcode{cauchy_distribution} object;}
\tcode{a} and \tcode{b}
correspond to the respective parameters of the distribution.
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\changed{\requires}{\expects} $0 < \tcode{m}$ and $0 < \tcode{n}$.

\pnum
\changed{\effects}{\remarks} \removed{Constructs a \tcode{fisher_f_distribution} object;}
\tcode{m} and \tcode{n}
correspond to the respective parameters of the distribution.
\end{itemdescr}
...
\pnum
\changed{\effects}{\remarks} \removed{Constructs a \tcode{student_t_distribution} object;}
\tcode{n} corresponds to the parameter of the distribution.
\end{itemdescr}
...
\end{removedblock}
 ...
 \changed{shall form}{forms} a sequence $w$ of length $n > 0$.
...
\pnum
\mandates
\tcode{is_invocable_r_v<double, UnaryOperation\&, double>} is true.
\end{addedblock}

\pnum \changed{\requires}{\expects}
 \removed{Each instance of type \tcode{UnaryOperation}
...
\pnum\complexity
 The number of invocations of \tcode{fw} \changed{shall}{does} not exceed $n$.
\end{itemdescr}
...
\mandates
\begin{itemize}
\item{\tcode{is_convertible_v<iterator_traits<InputIteratorB>::value_type, double>} is true.}
\item{\tcode{is_convertible_v<iterator_traits<InputIteratorW>::value_type, double>} is true.}
\end{itemize}
\end{addedblock}

\pnum \changed{\requires}{\expects}
  \tcode{InputIteratorB} and \tcode{InputIteratorW}
  \removed{shall} each meet the
  \oldconcept{InputIterator} requirements\iref{input.iterators}.
\begin{removedblock}
  Moreover,
...
\end{removedblock}
 ...
 \changed{shall form}{forms} a sequence $b$ of length $n+1$,
 the length of the sequence $w$ starting from \tcode{firstW}
 \changed{shall be}{is} at least $n$,
 and any $w_k$ for  $k \geq n$ \changed{shall be}{are} ignored by the distribution.
...
\end{removedblock}
 ...
 \changed{shall form}{forms} a sequence $b$ of length $n+1$,
 the length of the sequence $w$ starting from \tcode{firstW}
 \changed{shall be}{is} at least $n+1$,
 and any $w_k$ for $k \geq n + 1$ \changed{shall be}{are} ignored by the distribution.
...
\pnum\complexity
 The number of invocations of \tcode{fw} \changed{shall}{does} not exceed $n+1$.
\end{itemdescr}
...
\begin{addedblock}
\pnum
\expects \range{p}{p + n} is a valid range.
\end{addedblock}
...
\begin{itemdescr}
\pnum
\changed{\requires}{\expects} The length of the array to which the argument refers equals \tcode{size()}.
...
and returns a value which is of type \tcode{T} (\tcode{bool} for
...
\changed{\requires}{\expects}
\tcode{size() == v.size()}.
\removed{Each of these operators may only be instantiated for a type \tcode{T}
...
\changed{\requires}{\expects}
\tcode{size() > 0}.
\removed{This function may only be instantiated for a type \tcode{T} to which
\tcode{operator+=}
can be applied.}
...
\pnum
\mandates
The indicated operator can be applied to an operand of type \tcode{T} and returns 
a value which is of type \tcode{T} or which can be unambiguously
...
\mandates
The indicated operator can be applied to an operand of type \tcode{T} and returns 
a value which is of type \tcode{bool} or which can be unambiguously
...
This function returns a value which is of type \tcode{T}
or which can be unambiguously implicitly converted to type \tcode{T}.
\end{addedblock}
