...
\pnum
\mandates
\tcode{is_convertible_v<iterator_traits<InputIteratorB>::value_type, double> \&\& is_convertible_v<iterator_traits<InputIteratorW>::value_type, double>} is true.
\end{addedblock}
...
\pnum \changed{\requires}{\expects}
  \tcode{InputIteratorB} and \tcode{InputIteratorW}
  \changed{shall each satisfy}{each meet} the
  \oldconcept{InputIterator} requirements\iref{input.iterators}.
\begin{removedblock}
  Moreover,
  the \grammarterm{id-expression}s
