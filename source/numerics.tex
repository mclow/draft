...
\requires
  ...
  Moreover,
  \tcode{iterator_traits<InputIterator>::value_type}
  \changed{shall denote}{is} a type that is convertible to \tcode{double}.
...
\pnum \changed{\requires}{\expects}
 ...
 \changed{shall be}{is} a function object\iref{function.objects}
 whose return type \changed{shall be}{is} convertible to \tcode{double}.
 Moreover,
 \tcode{double} \changed{shall be}{is} convertible
...
\pnum \changed{\requires}{\expects}
  ...
  \changed{shall each denote a type that is}{are both types that are} convertible to \tcode{double}.
...
\pnum \changed{\requires}{\expects}
  ...
  \tcode{iterator_traits<InputIteratorB>::value_type} and
  \tcode{iterator_traits<Input\-IteratorW>::value_type}
  \changed{shall each denote a type that is}{are both} convertible to \tcode{double}.
 If \tcode{firstB == lastB}
 or \tcode{++firstB == lastB},
 let $n = 1$,
     $\rho_0 = \rho_1 = 1$,
...
\pnum \changed{\requires}{\expects}
 Each instance of type \tcode{UnaryOperation}
 \changed{shall be}{is} a function object\iref{function.objects}
 whose return type \changed{shall}{are} be convertible to \tcode{double}.
 Moreover,
 \tcode{double} \changed{shall be}{is} convertible
 to the type of \tcode{UnaryOperation}'s sole parameter.
...
\pnum
\constraints
\tcode{operator+=} be applied to operarands of type \tcode{T}.
\end{addedblock}
