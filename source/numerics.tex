...
\pnum
A class \tcode{S}
\changed{satisfies}{meets} the requirements
of a seed sequence
...
\pnum
A class \tcode{E}
that \changed{satisfies}{meets} the requirements
of a uniform random bit generator\iref{rand.req.urng}
also \changed{satisfies}{meets} the requirements
of a \term{random number engine}
if the expressions shown
in \tref{RandomEngine}
are valid and have the indicated semantics,
and if \tcode{E} also \changed{satisfies}{meets} all other requirements
...
\begin{itemize}
  ...
  \item
    \tcode{q} is an lvalue
    \changed{satisfying}{meeting} the requirements of a seed sequence\iref{rand.req.seedseq};
  \item
    \tcode{z} is a value
...
\pnum
\tcode{E} shall \changed{satisfy}{meet} the
...
also \changed{satisfy}{meet} all other requirements
...
\begin{itemize}
  ...
  \item
    ...
    \changed{satisfying}{that meet} the requirements
    of a uniform random bit generator\iref{rand.req.urng};
  \item
    \tcode{os} is an lvalue of the type of some class template specialization
    \tcode{basic_ostream<charT,} \tcode{traits>};
...
\pnum
\tcode{D} shall \changed{satisfy}{meet} the
\oldconcept{CopyConstructible} (\tref{copyconstructible})
and \oldconcept{CopyAssignable} (\tref{copyassignable}) requirements.
...
\pnum
\tcode{P} shall \changed{satisfy}{meet} the
\oldconcept{CopyConstructible} (\tref{copyconstructible}),
...
\mandates
  ...
  \changed{shall denote}{is} an integer type.
\end{addedblock}

\pnum
\changed{\requires}{\expects}
  \tcode{InputIterator} \changed{shall satisfy}{meets} the
...
\changed{\requires}{\expects} \tcode{RandomAccessIterator}
  \changed{shall satisfy}{meets} the
...
\pnum\changed{\requires}{\expects}
  \tcode{OutputIterator} \changed{shall satisfy}{meets} the
...
\pnum \changed{\requires}{\expects}
 ...
 whose return type \changed{shall be}{is} convertible to \tcode{double}.}
 \removed{Moreover,
...
\pnum \changed{\requires}{\expects}
  \tcode{InputIteratorB} and \tcode{InputIteratorW}
  \changed{shall each satisfy}{both meet} the
  \oldconcept{InputIterator} requirements\iref{input.iterators}.
  \removed{Moreover,
  the \grammarterm{id-expression}s
  \tcode{iterator_traits<InputIteratorB>::value_type} and
