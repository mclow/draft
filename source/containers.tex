...
\tcode{X u(i, j);}   &
 ...
 \added{\effects} Constructs a sequence container equal to the range \tcode{[i, j)}    \\ \rowsep
...
\tcode{a.\brk{}insert(\brk{}i, j)}          &
  ...
  \changed{\requires \tcode{i}, \tcode{j} are not iterators into \tcode{a}}{Neither \tcode{i} nor \tcode{j} are iterators into \tcode{a}.}.
...
the comparator \tcode{operator<} (for the first two overloads) or
\tcode{comp} (for the last two overloads)\changed{.}{, and \tcode{get_allocator() == x.get_allocator()}}.
