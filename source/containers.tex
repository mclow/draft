...
\tcode{a.\~X()}    &
 \tcode{void}               &
                            &
 \added{\effects}\changed{the destructor is applied to}{destroys} every element of \tcode{a}; any memory obtained is deallocated. &
 linear                     \\ \rowsep
...
\tcode{a.swap(b)}          &
 \tcode{void}               &
                            &
 \added{\effects}exchanges the contents of \tcode{a} and \tcode{b} &
 (Note A)                   \\ \rowsep
...
\tcode{a = rv}          &
  ...
  \added{\effects}All existing elements of \tcode{a}
  are either move assigned to or destroyed.\br
  \ensures \tcode{a} \changed{shall be}{is} equal to the value that \tcode{rv} had before
  this assignment.      &
  linear                \\ \rowsep
...
\tcode{X u(n, t);}   &
                &
 \changed{\requires}{\expects}\ \tcode{T} \changed{shall be}{is}
 \oldconcept{CopyInsertable} into \tcode{X}.\br
 \ensures \tcode{distance(begin(), end()) == n}\br
 \added{\effects} Constructs a sequence container with \tcode{n} copies of \tcode{t}  \\ \rowsep

\tcode{X(i, j)}\br
\tcode{X u(i, j);}   &
                    &
 \changed{\requires}{\expects}\ \tcode{T} \changed{shall be}{is} \oldconcept{EmplaceConstructible} into \tcode{X} from \tcode{*i}.
 For \tcode{vector}, if the iterator does
 not meet the \oldconcept{\-Forward\-Iterator} requirements\iref{forward.iterators}, \tcode{T}
 \changed {shall also be}{is also}
 \oldconcept{MoveInsertable} into \tcode{X}.
 \removed{Each iterator in the range \range{i}{j} shall be dereferenced exactly once.\br}
 \ensures \tcode{distance(begin(), end()) ==}
 \tcode{distance(i, j)}\br
 \added{\effects} Constructs a sequence container equal to the range \tcode{[i, j)}  
 \added{Each iterator in the range \range{i}{j} is dereferenced exactly once.}  \\ \rowsep
...
\tcode{a = il}     &
  ...
  \added{\effects} Assigns the range \range{il.begin()}{il.end()} into \tcode{a}. All existing
...
\tcode{a.insert(p,t)}   &
 ...
 \tcode{T} \changed {shall also be}{is also} \oldconcept{CopyAssignable}.\br
...
\tcode{a.insert(p,rv)}   &
 ...
 \oldconcept{MoveInsertable} into \tcode{X}. For \tcode{vector} and \tcode{deque},
  \tcode{T} \changed {shall also be}{is also} \oldconcept{MoveAssignable}.\br
...
\tcode{a.insert(p,n,t)}     &
 ...
 \oldconcept{CopyInsertable} into \tcode{X}
 and \oldconcept{CopyAssignable}.\br
 \added{\effects} Inserts \tcode{n} copies of \tcode{t} before \tcode{p}. \\ \rowsep

\tcode{a.insert(p,i,j)}    &
 \tcode{iterator}           &
 \changed{\requires}{\expects}\ \tcode{T} \changed{shall be}{is} \oldconcept{EmplaceConstructible} into \tcode{X} from \tcode{*i}.
 For \tcode{vector} and \tcode{deque}, \tcode{T} \changed {shall also be}{is also}
 \oldconcept{MoveInsertable} into \tcode{X}, \oldconcept{MoveConstructible}, \oldconcept{MoveAssignable},
 and swappable\iref{swappable.requirements}. \added{Neither \tcode{i} nor \tcode{j} are iterators into \tcode{a}.}
 \removed{Each iterator in the range \range{i}{j} shall be dereferenced exactly once.\br}

 \removed{\requires \tcode{i} and \tcode{j} are not iterators into \tcode{a}.}\br
 \added{\effects} Inserts copies of elements in \tcode{[i, j)} before \tcode{p} 
 \added{. Each iterator in the range \range{i}{j} is dereferenced exactly once.\br}  \\ \rowsep
...
\tcode{a.erase(q)}  &
 ...
 \tcode{T} \changed{shall be}{is} \oldconcept{MoveAssignable}.\br
...
\tcode{a.clear()}   &
 \tcode{void}       &
 \added{\effects}Destroys all elements in \tcode{a}. Invalidates all references, pointers, and
 iterators referring to the elements of \tcode{a} and may invalidate the past-the-end iterator. \ednote{Add linebreak here.}\br
 \ensures \tcode{a.empty()} \changed{returns}{is} \tcode{true}.\br
...
\tcode{a.assign(i,j)}   &
 ...
 \changed {shall also be}{is also}
 \oldconcept{MoveInsertable} into \tcode{X}. \added{Neither \tcode{i} nor \tcode{j} are iterators into \tcode{a}.}
 \removed{Each iterator in the range \range{i}{j} shall be dereferenced exactly once.\br}
 \removed{\requires \tcode{i}, \tcode{j} are not iterators into \tcode{a}.}\br
 \added{\effects}Replaces elements in \tcode{a} with a copy of \tcode{[i, j)}.\ednote{Remove linebreak here.}\br
...
\tcode{a.assign(n,t)}   &
 ...
 \added{\effects}Replaces elements in \tcode{a} with \tcode{n} copies of \tcode{t}.\ednote{Remove linebreak here.}\br
 Invalidates all references, pointers and iterators
 referring to the elements of \tcode{a}.
 For \tcode{vector} and \tcode{deque},
 also invalidates the past-the-end iterator.  \\
...
\tcode{a.emplace_\-front(args)}      &
 \tcode{reference}                &
 \added{\effects}Prepends an object of type \tcode{T} constructed with \tcode{std::forward<\brk{}Args\brk{}>(\brk{}args)...}.\br
 \changed{\requires}{\expects}\ \tcode{T} \changed{shall be}{is} \oldconcept{EmplaceConstructible} into \tcode{X} from \tcode{args}.\br
...
\tcode{a.emplace_\-back(args)}      &
 \tcode{reference}                &
 \added{\effects}Appends an object of type \tcode{T} constructed with \tcode{std::forward<\brk{}Args\brk{}>(\brk{}args)...}.\br
 \changed{\requires}{\expects}\ \tcode{T} \changed{shall be}{is} \oldconcept{EmplaceConstructible} into \tcode{X} from \tcode{args}. For \tcode{vector}, \tcode{T}
 \changed {shall also be}{is also}
 \oldconcept{MoveInsertable} into \tcode{X}.\br
 \returns{} \tcode{a.back()}.  &
 \tcode{deque},
...
\tcode{a.push_front(t)} &
  \tcode{void}          &
  \added{\effects}Prepends a copy of \tcode{t}.\br
  \changed{\requires}{\expects}\ \tcode{T} \changed{shall be}{is}
  \oldconcept{CopyInsertable} into \tcode{X}.
    &
  \tcode{deque},
...
\tcode{a.push_front(rv)} &
  \tcode{void}          &
  \added{\effects}Prepends a copy of \tcode{rv}.\br
  \changed{\requires}{\expects}\ \tcode{T} \changed{shall be}{is}
  \oldconcept{MoveInsertable} into \tcode{X}.
    &
  \tcode{deque},
...
\tcode{a.push_back(t)} &
  \tcode{void}          &
  \added{\effects}Appends a copy of \tcode{t}.\br
  \changed{\requires}{\expects}\ \tcode{T} \changed{shall be}{is}
  \oldconcept{CopyInsertable} into \tcode{X}.
    &
  \tcode{basic_string},
...
\tcode{a.push_back(rv)} &
  \tcode{void}          &
  \added{\effects}Appends a copy of \tcode{rv}.\br
  \changed{\requires}{\expects}\ \tcode{T} \changed{shall be}{is}
  \oldconcept{MoveInsertable} into \tcode{X}.
    &
  \tcode{basic_string},
...
\tcode{a.pop_front()}      &
 \tcode{void}               &
 \added{\effects}Destroys the first element.\br
 \changed{\requires}{\expects}\ \tcode{a.empty()} \changed{shall be}{is} \tcode{false}. &
 \tcode{deque},
 \tcode{forward_list},
 \tcode{list}
 \\ \rowsep
...
\tcode{a.pop_back()}       &
 \tcode{void}               &
 \added{\effects}Destroys the last element.\br
 \changed{\requires}{\expects}\ \tcode{a.empty()} \changed{shall be}{is} \tcode{false}. &
 \tcode{basic_string},
 \tcode{deque},
 \tcode{list},
...
\tcode{b.key_comp()}       &
 \tcode{X::key_compare}    &
 \changed{returns}{\returns} the comparison object out of which \tcode{b} was constructed. &
 constant                   \\ \rowsep

\indexordmem{value_comp}%
\tcode{b.value_comp()}     &
 \tcode{X::value_compare}  &
 \changed{returns}{\returns} an object of \tcode{value_compare} constructed out of the comparison object &
 constant                   \\ \rowsep

\indexordmem{emplace}%
\tcode{a_uniq.\brk{}emplace(\brk{}args)} &
 \tcode{pair<\brk{}iterator, bool>} &
  \changed{\requires}{\expects}\ \tcode{value_type} \changed{shall be}{is} \oldconcept{EmplaceConstructible} into \tcode{X} from \tcode{args}.\br
...
\tcode{a_uniq.\brk{}insert(\brk{}t)}      &
  \tcode{pair<\brk{}iterator, bool>}   &
  \changed{\requires}{\expects}\ If \tcode{t} is a non-const rvalue, \tcode{value_type} \changed{shall be}{is}
  \oldconcept{MoveInsertable} into \tcode{X}; otherwise, \tcode{value_type} \changed{shall be}{is}
  \oldconcept{CopyInsertable} into \tcode{X}.\br
...
\tcode{a.\brk{}insert(\brk{}i, j)}          &
  \tcode{void}                   &
  \changed{\requires}{\expects}\ \tcode{value_type} \changed{shall be}{is} \oldconcept{EmplaceConstructible} into \tcode{X} from \tcode{*i}. \ednote{Remove line break here.}
  \changed{\requires \tcode{i}, \tcode{j} are not iterators into \tcode{a}}{Neither \tcode{i} nor \tcode{j} are iterators into \tcode{a}}.\br
  \added{\effects} inserts each element from the range \range{i}{j} if and only if there
...
\tcode{a.\brk{}extract(\brk{}k)}              &
 \tcode{node_type}             &
 \added{\effects}removes the first element in the container with key equivalent to \tcode{k}.
 \changed{Returns}{\returns} a \tcode{node_type} owning the element if found, otherwise an empty
 \tcode{node_type}. &
 $\log (\tcode{a.size()})$       \\ \rowsep

\tcode{a.\brk{}extract(\brk{}q)}              &
 \tcode{node_type}             &
 \added{\effects}removes the element pointed to by \tcode{q}.
 \changed{Returns}{\returns} a \tcode{node_type} owning that element. &
...
\tcode{a.merge(a2)}              &
 ...
 \added{\effects}Attempts to extract each element in \tcode{a2} and insert it into \tcode{a}
...
\tcode{a.erase(k)}              &
 \tcode{size_type}             &
 \added{\effects} erases all elements in the container with key equivalent to
 \tcode{k}. \changed{Returns}{\returns} the number of erased elements.  &
 $\log (\tcode{a.size()}) + \tcode{a.count(k)}$       \\ \rowsep

\tcode{a.erase(q)}              &
 \tcode{iterator}               &
 \added{\effects} erases the element pointed to by \tcode{q}. \changed{Returns}{\returns} an iterator pointing to
...
\tcode{a.erase(r)}              &
 \tcode{iterator}               &
 \added{\effects} erases the element pointed to by \tcode{r}. \changed{Returns}{\returns} an iterator pointing to
...
\tcode{a.erase(}\br
 \tcode{q1, q2)}  &
 \tcode{iterator}        &
 \added{\effects} erases all the elements in the range \range{q1}{q2}. \changed{Returns}{\returns} an iterator pointing to
...
\tcode{a.clear()}       &
 \tcode{void}           &
 \changed{equivalent to}{\effects Equivalent to:} \tcode{a.erase(a.begin(),a.end())}\ednote{Remove line break here.}
 \ensures \tcode{a.empty()} \changed{returns}{is} \tcode{true}.  &
 linear in \tcode{a.size()}.  \\ \rowsep

\indexordmem{find}%
\tcode{b.find(k)}       &
 \tcode{iterator}; \tcode{const_iterator} for constant \tcode{b}.  &
 \changed{Returns}{\returns} an iterator pointing to an element with the key equivalent
...
\tcode{a_tran.}\br
 ...
 \changed{Returns}{\returns} an iterator pointing to an element with key \tcode{r} such that
...
\tcode{b.count(k)}        &
 \tcode{size_type}        &
 \changed{Returns}{\returns} the number of elements with key equivalent to \tcode{k}    &
 $\log (\tcode{b.size()}) + \tcode{b.count(k)}$   \\ \rowsep

\tcode{a_tran.}\br
 \tcode{count(ke)}        &
 \tcode{size_type}        &
 \changed{Returns}{\returns} the number of elements with key \tcode{r} such that
 \tcode{!c(r, ke) \&\& !c(ke, r)}    &
...
\tcode{b.}\br
 \tcode{contains(k)}  &
 \tcode{bool}         &
 \changed{equivalent to}{\effects Equivalent to: \tcode{return }} \tcode{b.find(k) != b.end()}  &
 logarithmic            \\ \rowsep

\tcode{a_tran.}\br
 \tcode{con\-tains(ke)} &
 \tcode{bool}         &
 \changed{equivalent to}{\effects Equivalent to: \tcode{return }} \tcode{a_tran.find(ke) != a_tran.end()}  &
 logarithmic            \\ \rowsep

\indexordmem{lower_bound}%
\tcode{b.lower_bound(k)}   &
 \tcode{iterator}; \tcode{const_iterator} for constant \tcode{b}.  &
 \changed{Returns}{\returns} an iterator pointing to the first element with
...
\tcode{b.equal_range(k)}       &
 ...
 \changed{equivalent to}{\effects Equivalent to: \tcode{return }} \tcode{make_pair(b.lower_bound(k), b.upper_bound(k))}.    &
 logarithmic                    \\ \rowsep
...
\tcode{a_tran.}\br
 ...
 \changed{equivalent to}{\effects Equivalent to: \tcode{return }} \tcode{make_pair(}\br
...
\tcode{X::hasher}
&   \tcode{Hash}
&   \added{\expects} \tcode{Hash} \changed{shall be}{is} a unary function object type such that the expression
...
&   \changed{\requires}{\expects}\ \tcode{Pred} \changed{is}{meets the} \oldconcept{CopyConstructible} \added{requirements}.\br
    \tcode{Pred} \changed{shall be}{is} a binary predicate that takes two arguments
...
&   \tcode{hasher}
&   \changed{Returns}{\returns} \tcode{b}'s hash function.%
&   constant
\\ \rowsep
%
...
\tcode{b.key_eq()}
&   \tcode{key_equal}
&   \changed{Returns}{\returns} \tcode{b}'s key equality predicate.%
&   constant
\\ \rowsep
%
...
\tcode{a.extract(k)}              &
 \tcode{node_type}             &
 \added{\effects}Removes an element in the container with key equivalent to \tcode{k}.
 \changed{Returns}{\returns} a \tcode{node_type} owning the element if found, otherwise an empty
 \tcode{node_type}. &
 Average case \bigoh{1}, worst case \bigoh{\tcode{a.size()}}.  \\ \rowsep
%
...
\tcode{a.erase(k)}
&   \tcode{size_type}
&   \added{\effects}Erases all elements with key equivalent to \tcode{k}.  \changed{Returns}{\returns}
...
\tcode{a.erase(q)}
&   \tcode{iterator}
&   \added{\effects}Erases the element pointed to by \tcode{q}. \changed{Returns}{\returns} the
...
\tcode{a.erase(r)}
&   \tcode{iterator}
&   \added{\effects}Erases the element pointed to by \tcode{r}. \changed{Returns}{\returns} the
...
\tcode{a.erase(q1, q2)}
&   \tcode{iterator}
&   \added{\effects}Erases all elements in the range \tcode{[q1, q2)}. \changed{Returns}{\returns}
...
\tcode{a.clear()}
& \tcode{void}
& \added{\effects}Erases all elements in the container.
   \ensures \tcode{a.empty()} \changed{returns}{is} \tcode{true}%
...
&   \changed{Returns}{\returns} an iterator pointing to an element with key equivalent to
...
&   \tcode{size_type}
&   \changed{Returns}{\returns} the number of elements with key equivalent to \tcode{ke}.%
&   Average case
...
&   \changed{Returns}{\returns} a range containing all elements with keys equivalent to
...
\tcode{b.bucket_count()}
&   \tcode{size_type}
&   \changed{Returns}{\returns} the number of buckets that \tcode{b} contains.%
&   Constant
\\ \rowsep
%
...
&   \tcode{size_type}
&   \changed{Returns}{\returns} an upper bound on the number of buckets that \tcode{b} might
    ever contain.%
&   Constant
\\ \rowsep
%
...
&
  \changed{\requires}{\expects} \tcode{b.bucket_count() > 0}.\br
    \changed{Returns}{\returns} the index of the bucket in which elements with keys equivalent
    to \tcode{k} would be found, if any such element existed.
    \ensures the return value \changed{shall be}{is} in the range \tcode{[0, b.bucket_count())}.%
& Constant
\\ \rowsep
%
...
&   \tcode{size_type}
&   \changed{\requires}{\expects} \tcode{n} \changed{shall be}{is} in the range \tcode{[0, b.bucket_count())}.\ednote{Add line break here} \br
    \changed{Returns}{\returns} the number of elements in the $\tcode{n}^\text{th}$ bucket.%
...
&   \changed{\requires}{\expects} \tcode{n} \changed{shall be}{is} in the range \tcode{[0, b.bucket_count())}.\ednote{Add line break here} \br
    \changed{\tcode{b.begin(n)} returns}{\returns} an iterator referring to the
...
&   \tcode{local_iterator}; \br
    \tcode{const_local_iterator} for const \tcode{b}.
&   \changed{\requires}{\expects} \tcode{n} \changed{shall be}{is} in the range \tcode{[0, b.bucket_count())}.\ednote{Add line break here} \br
    \changed{\tcode{b.end(n)} returns}{\returns} an iterator which is the past-the-end
...
&   \changed{\requires}{\expects} \tcode{n} \changed{shall be}{is} in the range \tcode{[0, b.bucket_count())}.\ednote{Add line break here} \br
    \changed{\tcode{b.cbegin(n)} returns}{\returns} an iterator referring to the
    first element in the bucket. If the bucket is empty, then
...
&   \tcode{const_local_iterator}
&   \changed{\requires}{\expects} \tcode{n} \changed{shall be}{is} in the range \tcode{[0, b.bucket_count())}.\ednote{Add line break here} \br
    \changed{\tcode{b.cend(n)} returns}{\returns} an iterator which is the past-the-end
    value for the bucket.%
&   Constant
\\ \rowsep
%
...
\tcode{b.load_factor()}
&   \tcode{float}
&   \changed{Returns}{\returns} the average number of elements per bucket.%
&   Constant
\\ \rowsep
%
...
&   \tcode{float}
&   \changed{Returns}{\returns} a positive number that the container attempts to keep the load factor
...
&   \tcode{void}
&   \changed{\requires}{\expects} \tcode{z} \changed{shall be}{is} positive.
...
\pnum
\changed{\requires}{\mandates} \tcode{(is_same_v<T, U> \&\& ...)} is \tcode{true}. \removed{Otherwise the program is ill-formed.}
...
\added{%
\pnum
\mandates \tcode{I < N} is \tcode{true}.
}
...
\pnum
\changed{\requires}{\expects} \tcode{T} \changed{shall be}{is} \oldconcept{DefaultInsertable} into \tcode{*this}.
...
\pnum
\changed{\requires}{\expects} \tcode{T} \changed{shall be}{is} \oldconcept{CopyInsertable} into \tcode{*this}.
...
\begin{addedblock}
\pnum
\expects \tcode{T} is \oldconcept{MoveInsertable} and \oldconcept{DefaultInsertable} into \tcode{*this}.
\end{addedblock}
...
\begin{itemdescr}
\pnum
\changed{\requires}{\expects} \tcode{T} \changed{shall be}{is} \oldconcept{MoveInsertable} into \tcode{*this}.
...
\changed{\requires}{\expects} \tcode{position} is \tcode{before_begin()} or is a dereferenceable
iterator in the range \range{begin()}{end()}.
\added{Neither} \tcode{first} \changed{and}{nor} \tcode{last} are \removed{not} iterators in \tcode{*this}.
...
\changed{\requires}{\expects} \tcode{position} is \tcode{before_begin()} or is a dereferenceable
iterator in the range \range{begin()}{end()}.
\tcode{get_allocator() == x.get_allocator()} \added{is \tcode{true}}.
\tcode{addressof(x) != this} \added{is \tcode{true}}.
...
\pnum
\remarks Stable\iref{algorithm.stable}. \removed{The behavior is undefined if
...
the comparator \tcode{operator<} (for the first two overloads) or
\tcode{comp} (for the last two overloads)\changed{.}{, and \tcode{get_allocator() == x.get_allocator()}} \added{is \tcode{true}}.
...
\pnum
\changed{\requires}{\expects}
\tcode{value_type} \changed{shall be}{is} \oldconcept{EmplaceConstructible} into \tcode{map}
...
\tcode{is_assignable_v<mapped_type\&, M\&\&>} \changed{shall be}{is} \tcode{true}.

\added{\pnum \expects}
\tcode{value_type} \changed{shall be}{is} \oldconcept{Emplace\-Constructible} into \tcode{map}
...
\pnum
\changed{\requires}{\expects}
\tcode{value_type} \changed{shall be}{is} \oldconcept{EmplaceConstructible} into \tcode{unordered_map}
...
\added{\pnum \expects}
\tcode{value_type} \changed{shall be}{is} \oldconcept{Emplace\-Constructible} into \tcode{unordered_map}
...
\pnum
\changed{\requires}{\expects}
\tcode{x} \changed{shall define}{defines} a strict weak ordering\iref{alg.sorting}.
...
\begin{itemdescr}
\pnum
\changed{\requires}{\expects} \range{ptr}{ptr + count} \changed{shall be}{is} a valid range.
If \tcode{extent} is not equal to \tcode{dynamic_extent},
then \tcode{count} \changed{shall be}{is} equal to \tcode{extent}.
...
\pnum
\changed{\requires}{\expects}
\range{first}{last} \changed{shall be}{is} a valid range.
If \tcode{extent} is not equal to \tcode{dynamic_extent},
...
\pnum
\changed{\requires}{\expects}
\range{data(cont)}{data(cont) + size(cont)} \changed{shall be}{is} a valid range.
If \tcode{extent} is not equal to \tcode{dynamic_extent},
...
\tcode{0 <= Count \&\& Count <= size()}\added{ is \tcode{true}}.
...
\pnum
\changed{\requires}{\expects}
\tcode{0 <= count 0 \&\& count <= size()}\added{ is \tcode{true}}.
...
\pnum
\changed{\requires}{\expects}
\tcode{0 <= idx \&\& idx < size()}\added{ is \tcode{true}}.
