...
All specializations of \tcode{fpos} \changed{satisfy}{meet}
...
The argument \tcode{fmt} \changed{shall be}{is} a valid pointer to an array of objects of type \tcode{charT} with \tcode{char_traits<charT>::length(fmt)} elements.
...
\tcode{struct tm}, and the argument \tcode{fmt} \changed{shall be}{is} a valid pointer to an
...
\pnum
\tcode{Allocator} shall \changed{satisfy}{meet} the \oldconcept{Allocator} requirements
...
\begin{floattable}
{Enum class \tcode{file_type}}{tab:fs.enum.file.type}
{lp{4.5in}}
\topline
\lhdr{Constant}	&
\rhdr{Meaning}	\\ \capsep
...
\pnum
 \tcode{directory_iterator} \changed{satisfies}{meets} the
