...
\pnum
\effects
\changed{Constructs an object of class
\tcode{failure} by constructing}{Constructs} the base class with \tcode{msg} and \tcode{ec}.
\end{itemdescr}
...
\pnum
\effects
\removed{Constructs an object of class \tcode{Init}.}
...
\pnum
\effects
\removed{Destroys an object of class
\tcode{Init}.}
...
\pnum
\remarks
Concurrent access to this function by multiple threads \changed{shall}{does} not result in a data
...
\changed{Constructs an object of class
\tcode{basic_ios},
assigning}{Assigns} initial values to its member objects by calling
\tcode{init(sb)}.
\end{itemdescr}
...
leaving}{Leaves} its member objects uninitialized. The object
...
\pnum
\changed{\requires}{\expects}
If \tcode{tiestr} is not null, \tcode{tiestr} \changed{shall not be}{is not} reachable by
...
\effects
If
\tcode{(this == addressof(rhs))} \added{is true}
does nothing.
...
\begin{itemdescr}
\pnum
\ensures \tcode{*this} \changed{shall have}{has} the state that
\tcode{rhs} had before the function call, except that
\tcode{rdbuf()} \changed{shall return 0}{returns \tcode{nullptr}}. \tcode{rhs} \changed{shall be}{is} in a valid but
unspecified state, except that \tcode{rhs.rdbuf()} \changed{shall return}{returns} the
same value as it returned before the function call, and
\tcode{rhs.tie()} \changed{shall return 0}{returns \tcode{nullptr}}.
\end{itemdescr}
...
\changed{shall be}{are} exchanged, except that \tcode{rdbuf()} \changed{shall return}{returns} the same
value as it returned before the function call, and \tcode{rhs.rdbuf()}
\changed{shall return}{returns} the same value as it returned before the function call.
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\changed{\requires}{\expects} \tcode{sb != nullptr} \added{is true}.
...
\pnum
\ensures \tcode{rdbuf() == sb} \added{is true}.
...
\changed{Constructs an object of class
\tcode{basic_streambuf<charT, traits>}
and initializes}{Initializes}:\footnote{The default constructor is protected for class
\tcode{basic_streambuf}
...
\tcode{gnext == gptr()},
and
\tcode{gend == egptr()} \added{are all \tcode{true}}.
\end{itemdescr}
...
\tcode{pbeg == pptr()},
and
\tcode{pend == epptr()} \added{are all \tcode{true}}.
\end{itemdescr}
...
\tcode{underflow()},
except that the result character \changed{shall be}{is} transferred from the pending
sequence to the backup sequence, and the pending sequence \changed{shall not
be}{is not} empty before the transfer.
...
\changed{shall obey}{obeys} the following constraints:
\begin{itemize}
\item
...
and
\tcode{pptr()}
\changed{shall be}{are} set so that:
\tcode{pptr() - pbase() == r}
...
\effects
\changed{Constructs an object of class
\tcode{basic_istream}, initializing}{Initializes} the base class subobject with
...
This is accomplished by default constructing}{Default constructs} the base class, \changed{copying}{copies} the
\tcode{gcount()} from \tcode{rhs}, \changed{calling}{calls}
\tcode{basic_ios<charT, traits>::move(rhs)} to initialize the base
class, and \changed{setting}{sets} the \tcode{gcount()} for \tcode{rhs} to 0.
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\effects \changed{As if by}{Equivalent to:} \tcode{swap(rhs)}.
...
\pnum
\changed{\requires}{\expects}
\tcode{s} \changed{shall not be}{is not} a null pointer.
...
The expression \tcode{out << resetiosflags(mask)} \changed{shall have}{has}
type \tcode{basic_ostream<charT, traits>\&} and value \tcode{out}. The
expression \tcode{in >> resetiosflags(mask)} \changed{shall have}{has} type
...
\begin{itemdescr}
\pnum
\changed{\requires}{\mandates} The type \tcode{moneyT} \changed{shall be}{is} either \tcode{long double} or a
...
The expression \tcode{in >> get_money(mon, intl)} \changed{shall have}{has} type
...
The expression \tcode{out << put_money(mon, intl)} \changed{shall have}{has} type
...
\begin{itemdescr}
\pnum
\changed{\requires}{\expects} The argument \tcode{tmb} \changed{shall be}{is} a valid pointer to an object of type \tcode{struct tm}
\changed{. The argument \tcode{fmt} shall be a valid pointer to an array of objects of type \tcode{charT} with \tcode{char_traits<charT>::length(fmt)} elements.}
{\tcode{, and [fmt, fmt + char_traits<charT>::length(fmt))} is a valid range.}
...
The expression \tcode{in >> get_time(tmb, fmt)} \changed{shall have}{has} type
\tcode{basic_istream<charT, traits>\&} and value \tcode{in}.
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\changed{\requires}{\expects} The argument \tcode{tmb} \changed{shall be}{is} a valid pointer to an object of type
\tcode{struct tm}, and \changed{the argument \tcode{fmt} shall be a valid pointer to an
array of objects of type \tcode{charT} with
\tcode{char_traits<charT>::length(\brk{}fmt)} elements}{\range{fmt}{fmt + char_traits<charT>::length(fmt)} is a valid range}.
...
The expression \tcode{out << put_time(tmb, fmt)} \changed{shall have}{has} type
\tcode{basic_ostream<charT, traits>\&} and value \tcode{out}.
\end{itemdescr}
...
The expression \tcode{out << quoted(s, delim, escape)} \changed{shall have}{has} type
\tcode{basic_ostream<charT, traits>\&} and value \tcode{out}.
\end{itemdescr}
...
\end{itemize}
The expression \tcode{in >> quoted(s, delim, escape)} \changed{shall have}{has} type
\tcode{basic_istream<charT, traits>\&} and value \tcode{in}. The expression
\tcode{out << quoted(s, delim, escape)} \changed{shall have}{has} type
...
\begin{itemdescr}
\pnum
\effects \changed{As if by}{Equivalent to:} \tcode{x.swap(y)}.
\end{itemdescr}
...
\changed{Constructs an object of class
\tcode{basic_filebuf<charT, traits>},
initializing}{Initializes} the base class with
...
\begin{itemdescr}
\pnum
\effects \removed{Move constructs from the rvalue \tcode{rhs}.} It
...
\pnum
\expects
\tcode{s} \changed{shall point}{points} to a NTCTS\iref{defns.ntcts}.
...
and \removed{initializing} \tcode{sb} with
...
\begin{itemdescr}
\pnum
\effects \changed{The same as}{Equivalent to:} \tcode{basic_ifstream(s.c_str(), mode)}.
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\effects \changed{Move constructs from the rvalue \tcode{rhs}. This
is accomplished by move constructing}{Move constructs} the base class, and the contained
...
\begin{itemdescr}
\pnum
\effects \changed{The same as}{Equivalent to: }\tcode{basic_ofstream(s.c_str(), mode)}.
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\effects \changed{The same as}{Equivalent to:} \tcode{basic_fstream(s.c_str(), mode)}.
\end{itemdescr}
...
\pnum
\effects
\changed{Constructs the \tcode{basic_syncbuf} object and
s}{S}ets \tcode{wrapped} to \tcode{obuf}.
...
\changed{\requires}{\expects} is not specified for the function.
...
\begin{itemdescr}
\pnum
\changed{\requires}{\expects} At most one element from each option group\iref{fs.enum.copy.opts}
...
\begin{itemdescr}
\pnum
\changed{\requires}{\expects} At most one element from each
...
\begin{itemdescr}
\pnum
  \changed{\requires}{\expects} Exactly one of the \tcode{perm_options} constants
