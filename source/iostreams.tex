...
The function
\tcode{fn}
\changed{shall}{does} not throw exceptions.
...
\tcode{rdbuf()} \changed{shall return}{returns} 0. \tcode{rhs} \changed{shall be}{is} in a valid but
...
\tcode{rhs.tie()} \changed{shall return}{returns} 0.
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\changed{\requires}{\expects} \tcode{sb != nullptr}.
...
\begin{addedblock}
\pnum
\constraints The expression \tcode{is >> std::forward<T>(x)} is well-formed.
\end{addedblock}
...
\begin{addedblock}
\pnum
\constraints The expression \tcode{os << x} is well-formed.
\end{addedblock}
...
\tcode{struct tm}, and \changed{the argument \tcode{fmt} \changed{shall be}{is} a valid pointer to an
array of objects of type \tcode{charT} with
\tcode{char_traits<charT>::length(\brk{}fmt)} elements}{\tcode{[fmt, fmt + char_traits<charT>::length(fmt))} is a valid range.}.
