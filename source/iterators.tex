...
\tcode{*r}          &
  unspecified       &
                            &
  \changed{\requires}{\expects} \tcode{r} is dereferenceable.  \\ \rowsep
...
\tcode{a != b}                  &
 ...
 \tcode{!(a == b)}                              &
 \changed{\requires}{\expects} \orange{a}{b} is in the domain of \tcode{==}. \\ \rowsep

\tcode{*a}                      &
 \tcode{reference}, convertible to \tcode{T}       &
                                &
 \changed{\requires}{\expects} \tcode{a} is dereferenceable.\br
...
\tcode{a->m}                    &
                                &
 \tcode{(*a).m}                                &
 \changed{\requires}{\expects} \tcode{a} is dereferenceable. \\ \rowsep
\tcode{++r}                     &
 \tcode{X\&}                    &
                                &
 \changed{\requires}{\expects} \tcode{r} is dereferenceable.\br
...
\tcode{\dcr r}      &
 \tcode{X\&}        &
                    &
 \changed{\requires}{\expects} there exists \tcode{s} such that \tcode{r == ++s}.\br
...
\tcode{r -= n}      &
 \tcode{X\&}        &
 \tcode{return r += -n;}    &
 \changed{\requires}{\expects} the absolute value of \tcode{n} is in the range of
...
\tcode{b - a}       &
 \tcode{difference_type}   &
 \tcode{return n}   &
 \changed{\requires}{\expects} there exists a value \tcode{n} of type \tcode{difference_type} such that \tcode{a + n == b}.\br
 \tcode{b == a + (b - a)}.  \\ \rowsep
...
\pnum
\constraints
\removed{The expression} \tcode{x.current == y.current} \changed{shall be valid}{is well-formed} and
convertible to \tcode{bool}.
...
\begin{itemdescr}
\added{\pnum \mandates \tcode{U} is convertible to \tcode{Iterator}.}

\pnum
\effects Constructs a \tcode{move_iterator}, initializing
\tcode{current} with \tcode{u.base()}.

\removed{\pnum \requires \tcode{U} shall be convertible to \tcode{Iterator}.}
\end{itemdescr}
...
\pnum
\constraints
\removed{The expression} \tcode{x.base() == y.base()} \changed{shall be valid}{is well-formed} and
convertible to \tcode{bool}.
...
\pnum
\constraints
\removed{The expression} \tcode{y.base() < x.base()} \changed{shall be valid}{is well-formed} and
convertible to \tcode{bool}.
...
\pnum
\constraints
\removed{The expression} \tcode{x + n} \changed{shall be valid}{is well-formed} and \changed{have}{has} type \tcode{Iterator}.
...
\begin{removedblock}
\pnum
\effects Constructs a copy of \tcode{x}.
...
\begin{itemdescr}
\pnum
\changed{\requires}{\expects} \tcode{in_stream != 0}\added{ is \tcode{true}}.
...
\tcode{s.rdbuf()}
\changed{shall not be}{is not} a null pointer.
