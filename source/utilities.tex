...
\pnum
\changed{\requires}{\expects}
\tcode{VT} \changed{shall satisfy}{meets} the \oldconcept{CopyConstructible} requirements.
}
...
\tcode{D} \changed{shall satisfy}{meets} the \oldconcept{MoveConstructible}
...
\begin{itemdescr}
\pnum
\changed{\requires}{\expects} If \tcode{D} is not a reference type, \tcode{D} \changed{shall satisfy}{meets} the
...
\tcode{remove_reference_t<D>} \changed{shall satisfy}{meets} the \oldconcept{CopyAssignable}
...
\tcode{A} \changed{shall satisfy}{meets} the \oldconcept{Allocator} requirements
...
\begin{itemdescr}
\pnum
\changed{\requires}{\expects} \tcode{A} \changed{shall satisfy}{meets} the \oldconcept{Allocator}
requirements (\tref{utilities.allocator.requirements}).
...
\pnum
\changed{\requires}{\expects}
\removed{\tcode{is_constructible_v<FD, F>} shall be \tcode{true}. For each $\tcode{T}_i$
...
$\tcode{w}_N$)}\iref{func.require} \changed{shall be}{is} a valid expression for some
...
as specified below, \changed{shall be}{are} neither \tcode{volatile} nor \tcode{const volatile}.
...
\mandates
\tcode{is_constructible_v<FD, F>} is \tcode{true}. For each $\tcode{T}_i$
in \tcode{BoundArgs}, \tcode{is_con\-structible_v<$\tcode{TD}_i$, $\tcode{T}_i$>} is \tcode{true}.
\end{addedblock}
...
\tcode{\placeholdernc{INVOKE}(fd, $\tcode{w}_1$, $\tcode{w}_2$, $\dotsc$, $\tcode{w}_N$)}
\changed{shall be}{is} a valid expression for some
values $\tcode{w}_1$, $\tcode{w}_2$, $\dotsc$, $\tcode{w}_N$, where
$N$ has the value \tcode{sizeof...(bound_args)}.
The cv-qualifiers \cv{} of the call wrapper \tcode{g},
as specified below, \changed{shall be}{are} neither \tcode{volatile} nor \tcode{const volatile}.
...
\pnum
\changed{\requires}{\expects}
The value type of \tcode{RandomAccessIterator1} \changed{shall satisfy}{meets} the \oldconcept{DefaultConstructible} requirements,
...
\pnum
\changed{\requires}{\expects}
The value type of \tcode{RandomAccessIterator1} \changed{shall satisfy}{meets} the \oldconcept{DefaultConstructible},
