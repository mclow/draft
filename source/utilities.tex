...
are not be selected by class template argument deduction\iref{over.match.class.deduct}.
\end{addedblock}
...
an rvalue of type \tcode{E} shall be well-formed and shall not throw an exception.
Otherwise, \tcode{E} is a reference type and assignment of the deleter from an lvalue
of type \tcode{E} is be well-formed and does not throw an exception.
\end{addedblock}
...
\begin{itemdescr}
\pnum
\changed{\requires}{\expects} The expression \tcode{const_cast<T*>((U*)nullptr)} \changed{shall
be}{is} well-formed.
