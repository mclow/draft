...
\begin{itemdescr}
\pnum
\added{\mandates \tcode{is_copy_assignable_v<first_type>} is \tcode{true}
...
\begin{itemdescr}
\pnum
\changed{\requires}{\mandates} \tcode{I < 2}. \removed{The program is ill-formed if \tcode{I} is out of bounds.}
...
\begin{itemdescr}
\pnum
\changed{\requires}{\expects} \tcode{Alloc} shall meet the
...
\begin{itemdescr}
\pnum
\changed{\remarks}{\expects} All specializations of \tcode{tuple_size} \removed{shall} meet the
...
\begin{itemdescr}
\pnum
\changed{\requires}{\expects} \tcode{Alloc} shall meet the \oldconcept{Allocator}
...
\begin{itemdescr}
\pnum
\changed{\requires}{\mandates} Lvalues of type $\tcode{T}_i$ shall be swappable\iref{swappable.requirements} and
...
\tcode{E} shall be well-formed and shall not throw an exception.
Otherwise, \tcode{E} is a reference type and construction of the deleter from an
lvalue of type \tcode{E} is be well-formed and does not throw an exception.
\end{addedblock}
...
\begin{itemdescr}
\pnum
\changed{\requires}{\expects} \tcode{get_deleter()} \changed{shall be}{meets the}
swappable\iref{swappable.requirements} \added{requirements} and \changed{shall
...
\pnum\changed{\requires}{\expects} \tcode{Y} \changed{shall be}{is} a complete type. The expression
...
\begin{itemdescr}
\pnum
\changed{\requires}{\expects} The expression \tcode{static_cast<T*>((U*)nullptr)}
...
\changed{shall be}{is} well formed and \changed{shall have}{has} well-defined behavior.
...
\pnum
\remarks
\changed{\remarks This assignment operator shall not participate in overload resolution unless}{\constraints}
\tcode{decay_t<F>} is
...
\tcode{template<size_t Len,\br
 ...
 \changed{\requires{}}{\expects} \tcode{Len} \changed{shall not be}{is not} zero. \tcode{Align} \changed{shall be}{is} equal to
