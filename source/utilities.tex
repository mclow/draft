...
\added{\constraints \tcode{is_move_constructible_v<T>} is \tcode{true} and
\tcode{is_move_assignable_v<T>} is \tcode{true}.}
...
Type
\tcode{T}
\changed{shall be}{meets the}
\oldconcept{MoveConstructible} (\tref{moveconstructible})
and
\oldconcept{MoveAssignable} (\tref{cpp17.moveassignable}) \added{ requirements}.
...
\pnum
\changed{\requires}{\expects}
\tcode{a[i]} \changed{shall be}{is} swappable with\iref{swappable.requirements} \tcode{b[i]}
...
\begin{itemdescr}
\pnum
\added{\mandates For the second overload, \tcode{is_lvalue_reference_v<T>} is \tcode{false}.}

\returns
\tcode{static_cast<T\&\&>(t)}.

\pnum
\removed{\remarks If the second form is instantiated with an lvalue reference type, the program is ill-formed.}
...
\begin{itemdescr}
\pnum
\removed{\remarks If this function is odr-used\iref{basic.def.odr}, the program is ill-formed.}

\added{\mandates This function is not odr-used\iref{basic.def.odr}.}
...
\pnum
\added{\mandates} \tcode{T} \changed{shall be}{is} an integer type.
...
\pnum
\removed{If \tcode{N} is negative the program is ill-formed.} The alias template
...
\begin{itemdescr}
\pnum
\added{\constraints \tcode{is_default_constructible_v<first_type>} is \tcode{true} and
...
\begin{itemdescr}
\pnum
\added{\constraints \tcode{is_copy_construct\-ible_v<first_type>} is \tcode{true} and
...
\pnum
\remarks
\removed{This constructor shall not participate in overload resolution
...
\begin{itemdescr}
\pnum
\added{\constraints \tcode{is_constructible_v<first_type, U1>} is \tcode{true} and
...
\pnum
\remarks
\removed{This constructor shall not participate in overload resolution unless
...
\begin{itemdescr}
\pnum
\added{\constraints \tcode{is_constructible_v<first_type, const U1\&>} is \tcode{true} and
...
\begin{addedblock}
\pnum
\mandates \tcode{is_constructible_v<first_type, Args1...>} is \tcode{true}
...
\pnum
\remarks
This operator \changed{shall be}{is} defined as deleted unless
...
\begin{itemdescr}
\pnum
\added{\constraints \tcode{is_assignable_v<first_type\&, const U1\&>} is \tcode{true}
...
\pnum
\removed{\remarks This operator shall not participate in overload resolution unless
...
\begin{itemdescr}
\pnum
\added{\constraints \tcode{is_move_assignable_v<first_type>} is \tcode{true}
...
\begin{itemdescr}
\pnum
\added{\constraints \tcode{is_assignable_v<first_type\&, U1>} is \tcode{true}
...
\pnum
\changed{\requires}{\expects}
\tcode{first} \changed{shall be}{is} swappable with\iref{swappable.requirements}
\tcode{p.first} and \tcode{second} \changed{shall be}{is} swappable with \tcode{p.second}.
...
\begin{itemdescr}
\pnum
\added{\constraints \tcode{is_swappable_v<T1>} is \tcode{true} and
\tcode{is_swappable_v<T2>} is \tcode{true}.}

\effects
\changed{As if by}{Equivalent to:} \tcode{x.swap(y)}.
...
\begin{itemdescr}
\pnum
\changed{\requires}{\mandates} $I < 2$ \removed{. The program is ill-formed if \tcode{I} is out of bounds.}
...
if \tcode{I == 1} returns a reference to \tcode{p.second}\removed{;
...
\begin{itemdescr}
\pnum
\changed{\requires}{\mandates} \tcode{T1} and \tcode{T2} are distinct types. \removed{Otherwise, the program is ill-formed.}
...
\begin{addedblock}
\pnum
\constraints{\tcode{is_default_construct\-ible_v<$\tcode{T}_i$>} is \tcode{true} for all $i$.}
\end{addedblock}
...
\begin{addedblock}
\pnum
\constraints \tcode{sizeof...(Types) >= 1} and \tcode{is_copy_constructible_v<$\tcode{T}_i$>} is \tcode{true} for all $i$.
\end{addedblock}
...
\begin{itemdescr}
\pnum
\changed{\requires}{\mandates} \tcode{is_copy_constructible_v<$\tcode{T}_i$>} is \tcode{true} for all $i$.
...
\begin{itemdescr}
\pnum
\changed{\requires}{\mandates} \tcode{is_move_constructible_v<$\tcode{T}_i$>} is \tcode{true} for all $i$.
...
\begin{addedblock}
\pnum
\constraints \tcode{sizeof...(Types) == 2},
...
\begin{itemdescr}
\pnum
\changed{\requires}{\expects} \tcode{Alloc} \changed{shall meet}{meets} the
...
\begin{addedblock}
\pnum
\constraints \tcode{is_move_assignable_v<$\tcode{T}_i$>} is \tcode{true} for all $i$.
\end{addedblock}
...
\begin{addedblock}
\pnum
\constraints \tcode{sizeof...(Types) == sizeof...(UTypes)} and
...
\begin{addedblock}
\pnum
\constraints \tcode{is_assignable_v<$\tcode{T}_i$\&, $\tcode{U}_i$> == true}
for all $i$ and \tcode{sizeof...(Types) == sizeof...(UTypes)}.
\end{addedblock}
...
\begin{addedblock}
\pnum
\constraints \tcode{sizeof...(Types) == 2} and
\tcode{is_assignable_v<$\tcode{T}_0$\&, const U1\&>} is \tcode{true} 
and \tcode{is_assignable_v<$\tcode{T}_1$\&, const U2\&>} is \tcode{true}.
\end{addedblock}
...
\tcode{is_assignable_v<$\tcode{T}_0$\&, const U1\&>} is \tcode{true}
\tcode{Types} and \tcode{is_assignable_v<$\tcode{T}_1$\&, const U2\&>} is \tcode{true}.
\end{removedblock}
...
\pnum
\constraints \tcode{sizeof...(Types) == 2} and
\tcode{is_assignable_v<$\tcode{T}_0$\&, U1>} is \tcode{true}
and \tcode{is_assignable_v<$\tcode{T}_1$\&, U2>} is \tcode{true}.
\end{addedblock}
...
\pnum
\changed{\requires}{\expects}
Each element in \tcode{*this} \changed{shall be}{is} swappable with\iref{swappable.requirements}
...
\pnum
\changed{\requires}{\expects} For all $i$, $\tcode{U}_i$ \changed{shall be}{is} the type
...
$\tcode{A}_{ik}$ the following requirements \changed{shall be}{are} met:
\begin{itemize}
...
\pnum
\remarks
The types in \tcode{CTypes} \changed{shall be}{are} equal to the ordered
...
\begin{itemdescr}
\pnum
\removed{\remarks} All specializations of \tcode{tuple_size} \removed{shall} meet the
...
of the three templates \removed{shall} meet the \oldconcept{UnaryTypeTrait} requirements\iref{meta.rqmts}
with a base characteristic of
\begin{codeblock}
integral_constant<size_t, TS::value>
\end{codeblock}
Otherwise, they \removed{shall} have no member \tcode{value}.
...
each of the three templates \removed{shall} meet the \oldconcept{TransformationTrait}
...
\begin{itemdescr}
\pnum
\changed{\requires}{\mandates} For all \tcode{i},
where \tcode{0 <= i} and
...
\begin{itemdescr}
\pnum
\changed{\requires}{\expects} \tcode{Alloc} \changed{shall meet}{meets} the \oldconcept{Allocator}
...
\begin{addedblock}
\pnum
\constraints \tcode{is_swappable_v<T>} is \tcode{true}
...
\pnum
\remarks
This constructor \changed{shall be}{is} defined as deleted unless
...
\begin{addedblock}
\pnum
\constraints \tcode{is_move_constructible_v<T>} is \tcode{true}.
\end{addedblock}
...
\begin{addedblock}
\pnum
\constraints \tcode{is_constructible_v<T, Args...>} is \tcode{true}.
\end{addedblock}
...
\pnum
\remarks
If \tcode{T}'s constructor selected for the initialization is a constexpr constructor, this constructor \changed{shall be}{is} a constexpr constructor.
\removed{This constructor shall not participate in overload resolution
...
\begin{addedblock}
\pnum
\constraints \tcode{is_constructible_v<T, initializer_list<U>\&, Args...>} is \tcode{true}.
\end{addedblock}
...
\pnum
\remarks
\removed{This constructor shall not participate in overload resolution unless \tcode{is_constructible_v<T, initializer_list<U>\&, Args\&\&...>} is \tcode{true}.}
If \tcode{T}'s constructor selected for the initialization is a constexpr constructor, this constructor \changed{shall be}{is} a constexpr constructor.
\end{itemdescr}
...
\begin{addedblock}
\pnum
\constraints \tcode{is_constructible_v<T, U>} is \tcode{true},
...
this constructor \changed{shall be}{is} a constexpr constructor.
\removed{This constructor shall not participate in overload resolution unless
...
\begin{addedblock}
\pnum
\constraints \tcode{is_move_constructible_v<T>} is \tcode{true} and
\tcode{is_move_assignable_v<T>} is \tcode{true}.
\end{addedblock}
...
\begin{addedblock}
\pnum
\constraints \tcode{is_same_v<remove_cvref_t<U>, optional>} is \tcode{false},
...
\remarks
If an exception is thrown during the call to \tcode{T}'s constructor, \tcode{*this} does not contain a value, and the previous \tcode{*val} (if any) has been destroyed.
\removed{This function shall not participate in overload resolution unless \tcode{is_constructible_v<T, initializer_list<U>\&, Args\&\&...>} is \tcode{true}.}
\end{itemdescr}
...
\begin{addedblock}
\pnum
\mandates \tcode{is_move_constructible_v<T>} is \tcode{true}.
\end{addedblock}

\pnum
\changed{\requires}{\expects}
Lvalues of type \tcode{T} \changed{shall be}{are} swappable \removed{and \tcode{is_move_constructible_v<T>} is \tcode{true}}.
...
\pnum
\remarks
These functions \changed{shall be}{are} constexpr functions.
\end{itemdescr}
...
\pnum
\remarks
This function \changed{shall be}{is} a constexpr function.
\end{itemdescr}
...
\begin{itemdescr}
\ednote{This has been removed in later drafts.}
\pnum
\effects
...
\pnum
\changed{\requires}{\mandates}
The expression \tcode{*x == *y} \changed{shall be}{is} well-formed and
its result \changed{shall be}{is} convertible to \tcode{bool}.
\begin{note}
...
Specializations of this function template
for which \tcode{*x != *y} is a core constant expression
\changed{shall be}{are} constexpr functions.
\end{itemdescr}
...
\pnum
\changed{\requires}{\mandates}
\tcode{*x < *y} \changed{shall be}{is} well-formed
and its result \changed{shall be}{is} convertible to \tcode{bool}.
...
\pnum
\changed{\requires}{\mandates}
The expression \tcode{*x == v} \changed{shall be}{is} well-formed and
its result \changed{shall be}{is} convertible to \tcode{bool}.
\begin{note}
\tcode{T} need not be \oldconcept{EqualityComparable}.
\end{note}
...
\pnum
\changed{\requires}{\mandates}
The expression \tcode{v == *x} \changed{shall be}{is} well-formed and
its result \changed{shall be}{is} convertible to \tcode{bool}.
...
\changed{shall evaluate}{evaluates} to the same value as \tcode{hash<remove_const_t<T>>()(*o)};
...
\begin{addedblock}
\pnum
\constraints \tcode{is_default_constructible_v<$\tcode{T}_0$>} is \tcode{true}.
\end{addedblock}
...
\begin{addedblock}
\pnum
\constraints There is exactly one occurrence of \tcode{T} in 
\tcode{Types...} and \tcode{is_constructible_v<T, Args...>} is \tcode{true}.
\end{addedblock}
...
constructor \changed{shall be}{is} a constexpr constructor.
\end{itemdescr}
...
\begin{addedblock}
\pnum
\constraints There is exactly one occurrence of \tcode{T} in \tcode{Types...} and
...
\begin{addedblock}
\pnum
\constraints \tcode{is_move_constructible_v<$\tcode{T}_i$> \&\&}
\tcode{is_move_assignable_v<$\tcode{T}_i$>} is \tcode{true} for all $i$.
\end{addedblock}
...
\begin{addedblock}
\pnum
\constraints \tcode{is_constructible_v<T, initializer_list<U>\&, Args...>} is
\tcode{true}, and \tcode{T} occurs exactly once in \tcode{Types}.
\end{addedblock}
...
\begin{addedblock}
\pnum
\constraints \tcode{is_constructible_v<$\tcode{T}_I$, Args...>} is \tcode{true}.
\end{addedblock}
...
\begin{addedblock}
\pnum
\constraints \tcode{is_constructible_v<$\tcode{T}_I$, initializer_list<U>\&, Args...>} is \tcode{true}.
\end{addedblock}
...
\begin{addedblock}
\pnum
\mandates \tcode{is_move_constructible_v<$\tcode{T}_i$>} is \tcode{true} for all $i$.
\end{addedblock}

\pnum
\changed{\requires}{\expects} Lvalues of type $\tcode{T}_i$ \changed{shall be}{are} swappable\iref{swappable.requirements}
\removed{and \tcode{is_move_constructible_v<$\tcode{T}_i$>} shall be \tcode{true} for all $i$}.
...
\pnum
\removed{\remarks}
All specializations of \tcode{variant_size} \changed{shall meet}{meets} the
...
type \tcode{T}. Then each of the three templates \changed{shall meet}{meets} the
\oldconcept{UnaryTypeTrait} requirements\iref{meta.rqmts} with a
...
cv-unqualified type \tcode{T}. Then each of the three templates \changed{shall
meet}{meets} the \oldconcept{TransformationTrait} requirements\iref{meta.rqmts} with a
...
\pnum
\changed{\requires}{\mandates}
\tcode{get<$i$>(v) == get<$i$>(w)} is a valid expression \removed{returning a type} that is
...
\pnum
\changed{\requires}{\mandates}
For each valid pack \tcode{m}, $e(\tcode{m})$ \changed{shall be}{is} a valid expression.
All such expressions \changed{shall be}{are} of the same type and value category\removed{;
otherwise, the program is ill-formed}.
...
\begin{addedblock}
\pnum
\constraints \tcode{is_move_constructible_v<$\tcode{T}_i$> \&\& is_swappable_v<$\tcode{T}_i$>}
is \tcode{true} for all $i$.
\end{addedblock}
...
\pnum
\remarks \removed{This function shall not participate in overload resolution
...
\begin{addedblock}
\pnum
\expects \tcode{VT} meets the \oldconcept{CopyConstructible} requirements.
\end{addedblock}
...
\pnum
\changed{\requires}{\expects} \tcode{VT} \changed{shall meet}{meets} the \oldconcept{CopyConstructible} requirements.
...
\pnum
\changed{\requires}{\expects}
\tcode{VT} \changed{shall meet}{meets} the \oldconcept{CopyConstructible} requirements.
...
\removed{Otherwise, the function constructs an object of class
\tcode{bitset<N>},}
\changed{initializing}{Initializes} the first \tcode{M} bit
...
\begin{addedblock}
\pnum
The function uses \tcode{traits::eq} to compare the character values.
...
characters in \tcode{str} beginning at position \tcode{pos} is
other than \tcode{zero} or \tcode{one}.
...
\pnum
\changed{\requires}{\expects}
\tcode{pos} \changed{shall be}{is} valid.
...
\begin{itemdescr}
\pnum
\changed{\requires}{\expects} \tcode{p} \changed{shall be}{is} a safely-derived
...
\changed{shall have}{has} been previously declared reachable, and \changed{shall be}{is}
live\iref{basic.life} from the time of the call until the last
\tcode{undeclare_reachable(p)} call on the object.

\pnum
\returns
A safely derived copy of \tcode{p} which \changed{shall compare}{compares} equal to \tcode{p}.
...
then it \changed{shall be}{is} entirely within a single allocated object. The object
\changed{shall be}{is} live until the corresponding \tcode{undeclare_no_pointers()} call.
\begin{note}
In
...
\begin{itemdescr}
\pnum
\changed{\requires}{\expects} The same range \changed{shall previously have}{has previously} been passed to
...
\pnum
\changed{\requires}{\expects}
\tcode{p} \changed{shall be}{is} a pointer value obtained from \tcode{allocate()}.
\tcode{n} \changed{shall equal}{equals} the value passed as the first argument
...
\pnum
\expects
\range{result}{(last - first)} \changed{shall}{do} not overlap with \range{first}{last}.
...
\pnum
\expects
\range{ofirst}{olast} \changed{shall}{do} not overlap with \range{ifirst}{ilast}.
...
\pnum
\expects
\range{result}{n} \changed{shall}{do} not overlap with \range{first}{n}.
...
\pnum
\expects
\range{ofirst}{olast} \changed{shall}{do} not overlap with
\range{ifirst}{n}.
...
\begin{addedblock}
\pnum
\constraints \tcode{U*} is implicitly convertible to \tcode{T*}
\end{addedblock}
...
\begin{addedblock}
\pnum
\mandates \tcode{T} is a complete type.
\end{addedblock}
...
\begin{addedblock}
\pnum
\constraints \tcode{U(*)[]} is convertible to \tcode{T(*)[]}.
\end{addedblock}
...
\begin{itemdescr}
\pnum
\changed{\requires}{\expects} \tcode{D} \changed{shall
meet}{meets} the \oldconcept{DefaultConstructible} requirements (\tref{cpp17.defaultconstructible}),
and that construction \changed{shall}{does} not throw an exception.
...
\begin{addedblock}
\pnum
\mandates This constructor is not selected by class template
...
\begin{addedblock}
\pnum
\mandates These constructors
are not selected by class template argument deduction\iref{over.match.class.deduct}.
\end{addedblock}

\pnum
\changed{\requires}{\expects} For the first constructor, if \tcode{D} is not a reference type,
\tcode{D} \changed{shall meet}{meets} the \oldconcept{CopyConstructible} requirements and
...
\removed{These constructors shall not participate in overload resolution
...
\tcode{D} \changed{shall meet}{meets} the \oldconcept{MoveConstructible}
requirements (\tref{cpp17.moveconstructible}).
Construction
of the deleter from an rvalue of type \tcode{D} \changed{shall}{does} not
throw an exception.
...
construction of the deleter from an rvalue of type
\tcode{E} is well-formed and does not throw an exception.
Otherwise, \tcode{E} is a reference type and construction of the deleter from an
lvalue of type \tcode{E} is well-formed and does not throw an exception.
\end{addedblock}
...
\begin{itemdescr}
\pnum
\changed{\requires}{\expects} The expression \tcode{get_deleter()(get())} \changed{shall be}{is} well-formed,
\changed{shall have}{has} well-defined behavior, and \changed{shall}{does} not throw exceptions.
\begin{note}
The
...
\pnum
\changed{\requires}{\expects} If \tcode{D} is not a reference type, \tcode{D} \changed{shall meet}{meets} the
\oldconcept{MoveAssignable} requirements (\tref{cpp17.moveassignable}) and assignment
of the deleter from an rvalue of type \tcode{D} \changed{shall}{does} not throw an exception.
Otherwise, \tcode{D} is a reference type;
\tcode{remove_reference_t<D>} \changed{shall meet}{meets} the \oldconcept{CopyAssignable}
requirements and assignment of the deleter from an
lvalue of type \tcode{D} \changed{shall}{does} not throw an exception.
...
an rvalue of type \tcode{E} is well-formed and does not throw an exception.
Otherwise, \tcode{E} is a reference type and assignment of the deleter from an lvalue
of type \tcode{E} is well-formed and does not throw an exception.
\end{addedblock}
...
\begin{itemdescr}
\pnum
\changed{\requires}{\expects} The expression \tcode{get_deleter()(get())}
\changed{shall be}{is} well-formed, \changed{shall have}{has}
well-defined behavior, and \changed{shall}{does} not throw exceptions.
...
\begin{itemdescr}
\pnum
\changed{\requires}{\expects} \tcode{get_deleter()} \changed{shall be}{is}
swappable\iref{swappable.requirements} and \changed{shall
not}{does not} throw an exception
under \tcode{swap}.
...
\tcode{less<CT>} \changed{shall be}{is} a function object type\iref{function.objects} that
...
\begin{itemdescr}
\pnum
\changed{\requires}{\expects} The specialization \tcode{less<unique_ptr<T, D>::pointer>} \changed{shall be}{is}
...
\begin{addedblock}
\pnum
\mandates \tcode{Y} is a complete type.

\pnum
\constraints When \tcode{T} is an array type, the expression \tcode{delete[] p} is well-formed and either
...
\pnum\changed{\requires}{\expects} \removed{\tcode{Y} shall be a complete type.} The expression
\tcode{delete[] p}, when \tcode{T} is an array type, or
\tcode{delete p}, when \tcode{T} is not an array type,
\changed{shall have}{has} well-defined behavior, and
\changed{shall not}{does not} throw exceptions.
...
\pnum
\changed{\requires}{\expects} Construction of \tcode{d} and a deleter of type \tcode{D}
initialized with \tcode{std::move(d)} \changed{shall not}{do not} throw exceptions.
The expression \tcode{d(p)}
\changed{shall have}{has} well-defined behavior and \changed{shall not}{does not} throw exceptions.
\tcode{A} \changed{shall meet}{meets} the \oldconcept{Allocator} requirements
(\tref{cpp17.allocator}).
...
\begin{itemdescr}
\pnum
\changed{\remarks This constructor shall not participate in overload resolution
...
\begin{itemdescr}
\pnum
\changed{\requires}{\expects} \tcode{get() != 0}.
...
\begin{itemdescr}
\pnum
\changed{\requires}{\expects} \tcode{get() != 0 \&\& i >= 0}.
...
\begin{itemdescr}
\pnum
\changed{\requires}{\expects} \tcode{A} \changed{shall meet}{meets} the \oldconcept{Allocator}
requirements (\tref{cpp17.allocator}).
...
\pnum
\remarks
\removed{These overloads shall only participate in overload resolution
when \tcode{T} is not an array type.}
...
\begin{itemdescr}
\pnum
\changed{\requires}{\mandates} The expression \tcode{static_cast<T*>((U*)nullptr)}
\changed{shall be}{is} well-formed.
...
\begin{itemdescr}
\pnum
\changed{\requires}{\mandates} The expression \tcode{dynamic_cast<T*>((U*)nullptr)}
\changed{shall be}{is} well-formed.
The expression \tcode{dynamic_cast<typename shared_ptr<T>::element_type*>(r.get())}
\changed{shall be}{is} well formed \removed{and shall have well-defined behavior}.
...
\begin{itemdescr}
\pnum
\changed{\requires}{\mandates} The expression \tcode{const_cast<T*>((U*)nullptr)} \changed{shall
be}{is} well-formed.
...
\begin{itemdescr}
\pnum
\changed{\requires}{\mandates} The expression \tcode{reinterpret_cast<T*>((U*)nullptr)}
\changed{shall be}{is} well-formed.
...
\begin{itemdescr}
\pnum
\changed{\remarks T}{\constraints For t}he second and third constructors
...
\begin{itemdescr}
\pnum
\changed{\remarks T}{\constraints For t}he second constructor
\changed{shall not participate in overload resolution unless}{,} \tcode{Y*} is compatible with \tcode{T*}.
...
\pnum
\tcode{operator()(x, y)} \changed{shall return}{returns} \tcode{x.owner_before(y)}.
\begin{note}
Note that
\begin{itemize}
...
\tcode{hash<UP>()(p)} \changed{shall evaluate}{evaluates} to
...
\tcode{hash<shared_ptr<T>>()(p)} \changed{shall evaluate}{evaluates} to
...
\pnum
\changed{\requires}{\expects}
\tcode{alignment} \changed{shall be}{is} a power of two.
...
\pnum
\changed{\requires}{\expects}
\tcode{p} \changed{shall have been}{was} returned from a prior call to \tcode{allocate(bytes, alignment)} on a memory resource equal to \tcode{*this},
and the storage at \tcode{p} \changed{shall not yet have}{has not yet} been deallocated.
...
\pnum
\changed{\requires}{\expects}
\tcode{upstream} \changed{shall be}{is} the address of a valid memory resource.
\tcode{initial_size}, if specified, \changed{shall be}{is} greater than zero.
...
\changed{\requires}{\expects}
\tcode{upstream} \changed{shall be}{is} the address of a valid memory resource.
\tcode{buffer_size} \changed{shall be}{is} no larger than the number of bytes in \tcode{buffer}.
...
\begin{addedblock}
\pnum
\constraints \tcode{is_constructible_v<OuterAlloc, OuterA2>} is \tcode{true}.
\end{addedblock}
...
\begin{addedblock}
\pnum
\constraints \tcode{is_constructible_v<OuterAlloc, const OuterA2\&>} is \tcode{true}.
\end{addedblock}
...
\changed{shall be}{are} equality-preserving\iref{concepts.equality}.
...
conversion sequences from both \tcode{T} and \tcode{U} to \tcode{P} \changed{shall be}{are}
...
\changed{shall be}{are} \tcode{true}.
...
\end{codeblock}
If they are not, they \changed{shall be}{are} declared as:
\begin{codeblock}
extern @\unspec@ _1;
\end{codeblock}%
\indextext{function object!binders|)}
...
\begin{itemdescr}
\pnum
\changed{\requires}{\expects} \tcode{F} \changed{shall be}{meets the} \oldconcept{CopyConstructible} \added{requirements}.

\pnum
\changed{\remarks This constructor template shall not participate in overload resolution unless}{\constraints}
...
\pnum
\remarks
\changed{This deduction guide participates in overload resolution only if
\tcode{\&F::operator()} is well-formed when treated as an unevaluated operand.
In that case, if}{If} \tcode{decltype(\&F::operator())} is of the form
...
\begin{addedblock}
\pnum
\constraints \tcode{decay_t<F>} is Lvalue-Callable\iref{func.wrap.func} 
...
\pnum
\changed{\requires}{\expects}
The value type of \tcode{RandomAccessIterator1} \changed{shall meet}{meets} the \oldconcept{DefaultConstructible} requirements,
...
if \tcode{pred(A, B) == true}, then \tcode{hf(A) == hf(B)} \changed{shall be}{is} \tcode{true}.
...
\pnum
\changed{\requires}{\mandates}
\tcode{RandomAccessIterator1} and \tcode{RandomAccessIterator2} \removed{shall} have the same value type.
...
\pnum
\changed{\requires}{\expects}
The value type of \tcode{RandomAccessIterator1} \changed{shall meet}{meets} the \oldconcept{DefaultConstructible},
...
\tcode{template<class T>\br
 ...
 \tcode{alignof(T)}.\br
 \changed{\requires{}}{\mandates}
 \tcode{alignof(T)} \changed{shall be}{is} a valid expression\iref{expr.alignof}  \\  \rowsep
...
\tcode{template<class T>}\br
 ...
 \changed{\requires{}}{\mandates} \tcode{T} \changed{shall be}{is} a (possibly cv-qualified)
...
\tcode{template<size_t Len,\br
 ...
 \changed{\requires{}}{\mandates} \tcode{Len} \changed{shall not be}{is not} zero. \tcode{Align} \changed{shall be}{is} equal to
...
\tcode{template<size_t Len,\br
 ...
 \changed{\requires{}}{\mandates} At least one type is provided.
  Each type in the template parameter pack \tcode{Types} \changed{shall be}{is} a complete object type.
  \\ \rowsep
...
\tcode{template<bool B, class T,}
 ...
 the member \tcode{type} \changed{shall be}{is} defined or omitted as specified in Note A, below.
...
\tcode{template<class Fn,}\br
 ...
 context'' and can result in the program being ill-formed.
 \end{note} \br
 \changed{\requires{}}{\expects} \tcode{Fn} and all types in the template parameter pack \tcode{ArgTypes}
 \changed{shall be}{are} complete types, \cv{}~\tcode{void}, or arrays of
 unknown bound.\\
\end{libreqtab2a}
...
specification are representable by \tcode{intmax_t}, the typedef \changed{shall be}{is}
defined; if either of the constants is not representable by \tcode{intmax_t},
the typedef \changed{shall not be}{is not} defined.
...
\pnum
\tcode{is_execution_policy<T>} \changed{shall be}{is} a \oldconcept{UnaryTypeTrait} with a
...
\tcode{terminate()} \changed{shall be}{is} called.
\end{itemdescr}
...
\begin{itemdescr}
\pnum
\changed{\requires}{\expects} \tcode{base} has a value between 2 and 36 (inclusive).
...
\begin{itemdescr}
\pnum
\changed{\requires}{\expects} \tcode{fmt} has the value of
