%!TEX root = std.tex
%%--------------------------------------------------
%% Title page for the C++ Standard


\thispagestyle{empty}
\begingroup
\def\hd{\begin{tabular}{ll}
          \textbf{Document Number:} & {\larger\docno}             \\
          \textbf{Date:}            & \reldate                    \\
          \textbf{Reply to:}        & Marshall Clow               \\
                                    & CppAlliance                 \\
                                    & mclow.lists@gmail.com
          \end{tabular}
}
\newlength{\hdwidth}
\settowidth{\hdwidth}{\hd}
\hfill\begin{minipage}{\hdwidth}\hd\end{minipage}
\endgroup

\vspace{2.5cm}
\begin{center}
\textbf{\Huge
Mandating the Standard Library:\\Clause 16 - Language support library}
\end{center}

With the adoption of P0788R3, we have a new way of specifying requirements for the
library clauses of the standard. This is one of a series of papers reformulating the
requirements into the new format. This effort was strongly influenced by the informational
paper P1369R0.

As a drive-by fix, I have removed a bunch of words of the form: "Effects: Constructs an object of class \tcode{bad_exception}."

The entire clause is reproduced here, but the changes are confined to a few sections:

\begin{itemize}
\item{support.types.byteops  \ref{support.types.byteops}}
\item{new.delete.single  \ref{new.delete.single}}
\item{new.delete.array  \ref{new.delete.array}}
\item{new.delete.placement  \ref{new.delete.placement}}
\item{bad.alloc  \ref{bad.alloc}}
\item{new.badlength  \ref{new.badlength}}
\item{ptr.launder  \ref{ptr.launder}}
\item{bad.cast  \ref{bad.cast}}
\item{bad.typeid  \ref{bad.typeid}}
\item{exception  \ref{exception}}
\item{bad.exception  \ref{bad.exception}}
\item{propagation  \ref{propagation}}
\item{except.nested  \ref{except.nested}}
\item{support.initlist.cons  \ref{support.initlist.cons}}
\end{itemize}

\vfill
\note{The changes here can be viewed as latex sources with the following commands:}
\begin{verbatim}
git clone git@github.com:mclow/mandate.git
cd mandate
git diff master..chapter16 support.tex
\end{verbatim}
\newpage
