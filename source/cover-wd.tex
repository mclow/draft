...
\item{atomics.ref.int}			\ref{atomics.ref.int}
\item{atomics.ref.float}		\ref{atomics.ref.float}
\item{atomics.ref.pointer}		\ref{atomics.ref.pointer}
\item{atomics.types.operations}	\ref{atomics.types.operations}
\item{atomics.types.float}		\ref{atomics.types.float}
...
\item{Qualified all the \tcode{memory_order} enum value in [atomics.ref.*] and [atomics.types.*]}
\end{itemize}

Open questions:
\begin{itemize}
\item{Should we change all the \tcode{memory_order::seq_cst} to \tcode{memory_order_seq_cst} (and others)?}
\end{itemize}
