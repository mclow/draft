...
\pnum
\changed{\requires}{\expects}
\tcode{a} and \tcode{b} \changed{shall be}{is} dereferenceable. \tcode{*a} \changed{shall be}{is}
...
\tcode{pred} \changed{shall}{is} be an equivalence relation and
...
\tcode{ranges::distance(r) > 0}.
For the overloads in namespace \tcode{std},
\tcode{T} \changed{shall be}{is} \oldconcept{Copy\-Constructible}.
For the first form, type \tcode{T} \changed{shall be}{meets the} \oldconcept{LessThanComparable}
\begin{addedblock}
requirements (\tref{lessthancomparable}).
\end{addedblock}
...
\pnum
\changed{\requires}{\expects}
\tcode{Cmp} \changed{shall be}{is} a function object type
...
\begin{itemize}
\item
  \tcode{T} \changed{shall be}{is} \oldconcept{MoveConstructible} (\tref{moveconstructible}).
\begin{removedblock}
\item
  All of \tcode{binary_op(init, *first)}, \tcode{binary_op(*first, init)},
  \tcode{binary_op(init, init)}, and \tcode{binary_op(*first, *first)}
...
\item
  ...
  \changed{shall be}{is} \oldconcept{MoveConstructible}.
\begin{removedblock}
\item
  If \tcode{init} is provided, all of
  \tcode{binary_op(init, init)},
...
\mandates
  ...
  are convertible to \tcode{ForwardIterator1}'s value type.
\end{addedblock}
...
\begin{itemize}
\item
  If \tcode{init} is provided, \tcode{T} \changed{shall be}{is}
  \oldconcept{MoveConstructible} (\tref{moveconstructible});
  otherwise, \tcode{ForwardIterator1}'s value type \changed{shall be}{is}
...
\item
  For the overloads with no \tcode{ExecutionPolicy},
  \tcode{T} \changed{shall be}{is} \oldconcept{MoveAssignable} (\tref{moveassignable}) and
