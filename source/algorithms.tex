...
\tcode{Function} \changed{shall satisfy}{meets}
...
\pnum
\changed{\requires}{\expects}
\tcode{Function} \changed{shall satisfy}{meets} the \oldconcept{CopyConstructible} requirements.
...
\pnum
\changed{\requires}{\expects}
\tcode{Function} \changed{shall satisfy}{meets} the \oldconcept{MoveConstructible} requirements
\begin{note}
...
\effects
Applies \tcode{f} to the result of dereferencing
every iterator in the range \range{first}{first + \changed{n}{$N$}} in order.
\begin{note}
If the type of \tcode{first} satisfies the requirements of a mutable iterator,
\tcode{f} may apply non-constant functions through the dereferenced iterator.
\end{note}
...
\effects
Applies \tcode{f} to the result of dereferencing
every iterator in the range \range{first}{first + \changed{n}{$N$}}.
\begin{note}
If the type of \tcode{first} satisfies the requirements of a mutable iterator,
\tcode{f} may apply non-constant functions through the dereferenced iterator.
\end{note}
...
\begin{addedblock}
\pnum
Let $N$ be $\max(0, \tcode{count})$.
\end{addedblock}

\pnum
\changed{\requires}{\mandates}
The type \tcode{Size}
\changed{shall be}{is} convertible to integral type~(\ref{conv.integral}, \ref{class.conv}).

\pnum
\returns
The first iterator \tcode{i} in the range \range{first}{last-\changed{count}{$N$}}
such that for every non-negative integer \tcode{n} less than \tcode{\changed{count}{$N$}}
the following corresponding conditions hold:
...
\pnum
\changed{\requires}{\expects}
\tcode{result} \changed{shall}{is} not be in the range \range{first}{last}.
...
\begin{itemdescr}
\pnum
Let \changed{$M$ be $\max(\tcode{n}, 0)$}{$N$ be $\max(0, \tcode{n})$}.
...
\pnum
\effects
For each non-negative integer \changed{$i < M$}{$i < N$},
...
\begin{itemize}
\item
  \tcode{result + \changed{$M$}{$N$}}
  for the overloads in namespace \tcode{std}, or
\item
  \tcode{\{first + \changed{$M$}{$N$}, result + \changed{$M$}{$N$}\}}
  for the overload in namespace \tcode{ranges}.
\end{itemize}

\pnum
\complexity
Exactly \changed{$M$}{$N$} assignments.
\end{itemdescr}
...
\pnum
\changed{\requires}{\expects}
\tcode{result} \changed{shall not be}{is not} in the range \brange{first}{last}.
...
\pnum
\changed{\requires}{\expects}
\tcode{result} \changed{shall not be}{is not} in the range \range{first}{last}.
...
\changed{shall}{do} not overlap.
For the overloads in namespace \tcode{std},
\tcode{*(first1 + $n$)} \changed{shall be}{is} swappable with\iref{swappable.requirements}
\tcode{*(first2 + $n$)}.
...
\pnum
\changed{\requires}{\expects}
\tcode{a} and \tcode{b} \changed{shall be}{are} dereferenceable. \tcode{*a} \changed{shall be}{is}
...
\pnum
\changed{\requires}{\expects}
\tcode{op} and \tcode{binary_op} \changed{shall}{do} not invalidate iterators or subranges, nor
...
\pnum
\changed{\requires}{\expects}
The expression \tcode{*first = new_value} \changed{shall be}{is} valid.
...
\changed{shall be}{are} writable\iref{iterator.requirements.general}
...
\pnum
\changed{\requires}{\mandates}
\tcode{Size} \changed{shall be}{is} convertible
to an integral type~(\ref{conv.integral}, \ref{class.conv}).
...
\changed{shall meet}{meets} the \oldconcept{MoveAssignable} requirements (\tref{moveassignable}).
...
\changed{\requires}{\expects}
The ranges \range{first}{last} and \range{result}{result + (last - first)}
\changed{shall}{do} not overlap. The expression \tcode{*result = *first} \changed{shall be}{is} valid.
\begin{note}
For the overloads with an \tcode{ExecutionPolicy},
there may be a performance cost
...
\tcode{pred} \changed{shall be}{is} an equivalence relation and
the type of \tcode{*first} \changed{shall meet}{meets}
...
\item
  ...
  \begin{itemize}
  \item
    The comparison function \changed{shall be}{is} an equivalence relation.
  \item
    The expression \tcode{*result = *first} \changed{shall be}{is} valid.
  \item
    For the overloads with no \tcode{ExecutionPolicy},
    ...
    then \tcode{T} \changed{shall meet}{meets}
    the \oldconcept{CopyAssignable} (\tref{copyassignable}) requirements.
    Otherwise, \tcode{T} \changed{shall meet}{meets} both
...
\changed{\requires}{\expects}
For the overloads in namespace \tcode{std},
\tcode{BidirectionalIterator} \changed{shall meet}{meets}
...
\pnum
\changed{\requires}{\expects}
\range{first}{middle} and \range{middle}{last} \changed{shall be}{are} valid ranges.
For the overloads in namespace \tcode{std},
\tcode{ForwardIterator} \changed{shall meet}{meets}
...
\begin{itemize}
\item
  \tcode{PopulationIterator} \changed{shall satisfy}{meets}
  the \oldconcept{InputIterator} requirements\iref{input.iterators}.
\item
  \tcode{SampleIterator} \changed{shall satisfy}{meets}
...
\item
  \tcode{PopulationIterator}'s value type \changed{shall be}{is}
...
\end{removedblock}
\item
  \tcode{remove_reference_t<UniformRandomBitGenerator>} \changed{shall satisfy}{meets}
...
\item
  \tcode{out} \changed{shall not be}{is not} in the range \range{first}{last}.
\end{itemize}
...
\begin{itemize}
\item
  Stable if and only if \tcode{PopulationIterator} \changed{satisfies}{meets}
  the \oldconcept{ForwardIterator} requirements.
\item
...
\begin{itemize}
\item
  \tcode{RandomAccessIterator} \changed{shall meet}{meets}
  the \oldconcept{ValueSwappable} requirements\iref{swappable.requirements}.
\item
  The type \tcode{remove_reference_t<UniformRandomBitGenerator>} \changed{shall meet}{meets}
...
\pnum
\changed{\requires}{\expects}
The type of \tcode{*first} \changed{shall satisfy}{meets}
...
for each non-negative integer \tcode{i < (last - first) - n}.
In the first overload case, if \tcode{ForwardIterator} \changed{satisfies}{meets}
...
\tcode{RandomAccessIterator} \changed{shall meet}{meets}
the \oldconcept{Value\-Swappable} requirements\iref{swappable.requirements},
the type of \tcode{*result_first} \changed{shall meet}{meets}
the \oldconcept{MoveConstructible} (\tref{moveconstructible}) and
\oldconcept{\-Move\-Assignable} (\tref{moveassignable}) requirements,
and the expression \tcode{*first}
\changed{shall be}{is} writable\iref{iterator.requirements.general} to \tcode{result_first}.
...
\pnum
\changed{\requires}{\expects}
\range{first}{nth} and \range{nth}{last} \changed{shall be}{are} valid ranges.
For the overloads in namespace \tcode{std},
\tcode{RandomAccessIterator} \changed{shall meet}{meets}
the \oldconcept{ValueSwappable} requirements\iref{swappable.requirements}, and
the type of \tcode{*first} \changed{shall meet}{meets}
...
\changed{shall be}{are} partitioned with respect to the expression
...
\changed{\requires}{\expects}
The elements \tcode{e} of \range{first}{last}
\changed{shall be}{are} partitioned with respect to the expressions
\tcode{bool(invoke(comp, invoke(proj, e), value))} and
\tcode{!bool(invoke(comp, value, invoke(proj, e)))}.
Also, for all elements \tcode{e} of \tcode{[first, last)},
\tcode{bool(comp(e, value))} \changed{shall imply}{implies} \tcode{!bool(comp(\brk{}value, e))}
for the overloads in namespace \tcode{std}.
...
\pnum
\changed{\requires}{\expects}
The input range and output ranges \changed{shall}{do} not overlap.
For the overloads in namespace \tcode{std},
the expression \tcode{*first}
\changed{shall be}{is} writable\iref{iterator.requirements.general}
...
\changed{\requires}{\expects}
The elements \tcode{e} of \range{first}{last}
\changed{shall be}{are} partitioned with respect to $E(\tcode{e})$.
...
\changed{shall be}{are} sorted with respect to \tcode{comp} and \tcode{proj1} or \tcode{proj2},
respectively.
The resulting range \changed{shall}{does} not overlap with either of the original ranges.
...
\pnum
\changed{\requires}{\expects}
The ranges \range{first1}{last1} and \range{first2}{last2} \changed{shall be}{are} sorted
...
The ranges \range{first1}{last1} and \range{first2}{last2} \changed{shall be}{are} sorted
with respect to \tcode{comp} and \tcode{proj1} or \tcode{proj2}, respectively.
\removed{The resulting range shall not overlap with either of the original ranges.}
...
\changed{shall be}{is} a valid heap with respect to \tcode{comp} and \tcode{proj}.
For the overloads in namespace \tcode{std},
the type of \tcode{*first} \changed{shall meet}{meets}
...
\changed{shall be}{is} a valid non-empty heap with respect to \tcode{comp} and \tcode{proj}.
For the overloads in namespace \tcode{std},
\tcode{RandomAccessIterator} \changed{shall meet}{meets}
the \oldconcept{ValueSwappable} requirements\iref{swappable.requirements} and
the type of \tcode{*first} \changed{shall meet}{meets}
...
\pnum
\changed{\requires}{\expects}
The range \range{first}{last} \changed{shall be}{is}
...
\pnum
\changed{\requires}{\expects}
For the first form, type \tcode{T} \changed{shall be}{meets the}
\oldconcept{LessThanComparable} \added{requirements} (\tref{lessthancomparable}).
...
For the overloads in namespace \tcode{std},
\tcode{T} \changed{shall be}{meets the} \oldconcept{Copy\-Constructible} \added{requirements}.
For the first form, type \tcode{T} \changed{shall be}{meets the} \oldconcept{LessThanComparable}
\begin{addedblock}
requirements (\tref{lessthancomparable}).
\end{addedblock}
...
\pnum
\changed{\requires}{\expects}
The value of \tcode{lo} \changed{shall be}{is} no greater than \tcode{hi}.
For the first form, type \tcode{T}
\changed{shall be}{meets the} \oldconcept{LessThan\-Comparable}
\added{requirements} (\tref{lessthancomparable}).
...
\pnum
\changed{\requires}{\expects}
\tcode{T} \changed{shall satisfy}{meets}
...
\tcode{binary_op} \changed{shall neither modify elements
nor invalidate iterators or subranges}{neither modifies elements
...
\begin{itemize}
\item
  \tcode{T} \changed{shall be}{meets the} \oldconcept{MoveConstructible} (\tref{moveconstructible}) \added{requirements}.
\begin{removedblock}
\item
  All of \tcode{binary_op(init, *first)}, \tcode{binary_op(*first, init)},
  \tcode{binary_op(init, init)}, and \tcode{binary_op(*first, *first)}
...
\end{removedblock}
\item
  \tcode{binary_op} \changed{shall neither invalidate}{neither invalidates} iterators or subranges,
  nor \changed{modify}{modifies} elements in the range \crange{first}{last}.
\end{itemize}
...
\changed{shall neither modify elements nor invalidate iterators or subranges}
...
\item
  ...
  \changed{shall invalidate}{invalidates} subranges, nor \changed{modify}{modifies} elements in the ranges
...
\end{removedblock}
\item
  Neither \tcode{unary_op} nor \tcode{binary_op} \changed{shall invalidate}{invalidates} subranges,
  nor \changed{modify}{modifies} elements in the range \crange{first}{last}.
\end{itemize}
...
\pnum
\mandates
\tcode{InputIterator}'s value type is constructible
from the type of \tcode{*first}.
The result of the
expression \tcode{std::move(acc) + *i} or \tcode{binary_op(std::move(acc), *i)}
is implicitly convertible to \tcode{InputIterator}'s value type.
\end{addedblock}

\pnum
\changed{\requires}{\expects}
\removed{\tcode{InputIterator}'s value type shall be constructible
from the type of \tcode{*first}.
The result of the
expression \tcode{std::move(acc) + *i} or \tcode{binary_op(std::move(acc), *i)}
shall be implicitly convertible to \tcode{InputIterator}'s value type.}
\tcode{acc} \changed{shall be}{is} writable\iref{iterator.requirements.general} to
...
\item
  \tcode{binary_op} \changed{shall neither invalidate}{neither invalidates} iterators or subranges,
  nor \changed{modify}{modifies} elements in
...
\mandates
  ...
  is convertible to \tcode{ForwardIterator1}'s value type.
\end{addedblock}
...
\item
  ...
  \changed{shall be}{meets the} \oldconcept{MoveConstructible} \added{requirements}.
\begin{removedblock}
\item
  If \tcode{init} is provided, all of
  \tcode{binary_op(init, init)},
...
\item
  ...
  \changed{shall invalidate}{invalidates} iterators or subranges, nor \changed{modify}{modifies} elements in
  the ranges \crange{first}{last} or \crange{result}{result + (last - first)}.
\end{itemize}
...
\mandates
  ...
  \changed{are}{is} convertible to \tcode{ForwardIterator1}'s value type.
\end{addedblock}
...
\begin{itemize}
\item
  If \tcode{init} is provided, \tcode{T} \changed{shall be}{meets the}
  \oldconcept{MoveConstructible} (\tref{moveconstructible}) \added{requirements};
  otherwise, \tcode{ForwardIterator1}'s value type \changed{shall be}{meets the}
  \oldconcept{MoveConstructible} \added{requirements}.
\begin{removedblock}
\item
  If \tcode{init} is provided, all of
  \begin{itemize}
...
\end{removedblock}
\item
  Neither \tcode{unary_op} nor \tcode{binary_op} \changed{shall invalidate}{invalidates}
  iterators or subranges, nor \changed{modify}{modifies} elements in
  the ranges \crange{first}{last} or \crange{result}{result + (last - first)}.
\end{itemize}
...
\item
  For the overloads with no \tcode{ExecutionPolicy},
  \tcode{T} is constructible from \removed{the type of} \tcode{*first}.
  ...
  is writable to the \tcode{result} output iterator.
\item
  For the overloads with an \tcode{ExecutionPolicy},
...
\item
  For the overloads with no \tcode{ExecutionPolicy},
  \tcode{T} \changed{shall be}{meets the} \oldconcept{MoveAssignable} (\tref{moveassignable}) \added{requirements} and
  \changed{shall be}{is} constructible from \removed{the type of} \tcode{*first}.
  \tcode{acc} (defined below) \changed{shall be}{is}
  writable\iref{iterator.requirements.general}
  to the \tcode{result} output iterator.
  The result of the expression \tcode{binary_op(val, std::move(acc))}
  \changed{shall be}{is} writable to the \tcode{result} output iterator.
\item
  For the overloads with an \tcode{ExecutionPolicy},
  the result of the expressions \tcode{binary_op(*first, *first)} and
  \tcode{*first} \changed{shall be}{is} writable to \tcode{result}.
\item
  For all overloads, in the ranges \crange{first}{last}
  and \crange{result}{result + (last - first)},
  \tcode{binary_op} \changed{shall neither modify}{neither modifies} elements
...
\pnum
\changed{\requires}{\mandates}
\tcode{T} \changed{shall be}{is} convertible to \tcode{ForwardIterator}'s value type.
...
\changed{\requires}{\expects}
$|\tcode{m}|$ and $|\tcode{n}|$
\changed{shall be}{are} representable as a value of \tcode{common_type_t<M, N>}.
\begin{note}
...
\pnum
\mandates
\tcode{M} are \tcode{N} both are integer types and not 
\cv{}~\tcode{bool}.
\end{addedblock}
...
\changed{shall be}{is} representable as a value of type \tcode{common_type_t<M,N>}.
...
unless the}{The} objects in the array pointed to by \tcode{base} are of trivial type.
