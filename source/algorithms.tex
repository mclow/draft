...
\tcode{Function} \changed{shall meet}{meets}
...
\pnum
\changed{\requires}{\expects}
\tcode{Function} \changed{shall meet}{meets} the \oldconcept{CopyConstructible} requirements.
...
\pnum
\changed{\requires}{\expects}
\tcode{Function} \changed{shall meet}{meets} the \oldconcept{MoveConstructible} requirements
\begin{note}
...
\pnum
\changed{\requires}{\mandates}
The expression \tcode{*first = new_value} \changed{shall be}{is} \changed{valid}{well-formed}.
...
are writable\iref{iterator.requirements.general}
to the \tcode{result} output iterator.
\end{addedblock}

\pnum
\changed{\requires}{\expects}
\removed{The results of the expressions \tcode{*first} and \tcode{new_value}
...
\changed{shall be}{is} writable\iref{iterator.requirements.general} to the output iterator.
The type \tcode{Size} \changed{shall be}{is} convertible
to an integral type~(\ref{conv.integral}, \ref{class.conv}).
...
\changed{shall meet}{meets} the \oldconcept{MoveAssignable} requirements (\tref{cpp17.moveassignable}).
...
\pnum
\mandates
The expression \tcode{*result = *first} is well-formed.
\end{addedblock}

\pnum
\changed{\requires}{\expects}
The ranges \range{first}{last} and \range{result}{result + (last - first)}
\changed{shall}{do} not overlap. \removed{The expression \tcode{*result = *first} shall be valid.}
\begin{note}
For the overloads with an \tcode{ExecutionPolicy},
there may be a performance cost
...
\item
  ...
  \item
    \removed{The expression \tcode{*result = *first} shall be valid.}
  \item
    For the overloads with no \tcode{ExecutionPolicy},
...
\pnum
\mandates
\tcode{Distance} is an integer type.   \tcode{PopulationIterator}'s value type is
...
\begin{itemize}
\item
  \tcode{PopulationIterator} \changed{shall meet}{meets}
  the \oldconcept{InputIterator} requirements\iref{input.iterators}.
\item
  \tcode{SampleIterator} \changed{shall meet}{meets}
...
\item
  \tcode{remove_reference_t<UniformRandomBitGenerator>} \changed{shall meet}{meets}
...
\removed{, and the expression \tcode{*first}
...
\removed{For the overloads in namespace \tcode{std},
the expression \tcode{*first} shall be writable\iref{iterator.requirements.general}
to \tcode{out_true} and \tcode{out_false}.}
\begin{note}
For the overload with an \tcode{ExecutionPolicy},
...
\oldconcept{LessThanComparable} \added{requirements} (\tref{cpp17.lessthancomparable}).
...
\added{requirements} (\tref{cpp17.lessthancomparable}).
...
\begin{itemize}
\item
  \tcode{T} \changed{shall be}{meets the} \oldconcept{MoveConstructible} (\tref{cpp17.moveconstructible}) \added{requirements}.
\begin{removedblock}
\item
...
\pnum
\changed{\requires}{\expects}
\tcode{T} \changed{shall meet}{meets}
the \oldconcept{CopyConstructible} (\tref{cpp17.copyconstructible})
and \oldconcept{CopyAssignable} (\tref{cpp17.copyassignable}) requirements.
...
\tcode{acc} is writable\iref{iterator.requirements.general} to
...
\item
  ...
  \oldconcept{MoveConstructible} (\tref{cpp17.moveconstructible}) \added{requirements};
...
\item
  For the overloads with no \tcode{ExecutionPolicy},
  \tcode{T} \changed{shall be}{meets the} \oldconcept{MoveAssignable} (\tref{cpp17.moveassignable}) \added{requirements} and
  \changed{shall be}{is} constructible from the type of \tcode{*first}.
...
\item
  For the overloads with no \tcode{ExecutionPolicy},
  \tcode{T} \changed{shall be}{meets the} \oldconcept{MoveAssignable} (\tref{moveassignable})
  \added{requirements} \removed{and
  is constructible from the type of \tcode{*first}.
